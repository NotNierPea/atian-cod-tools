...
namespace actslib::data {
    ...
    class KMerger {
      ...
      public:
        ...
        inline std::filesystem::path GetNewIdSyncPath() { return workdir / va("c_%lld", GetNewIdSync()); }
            ...
            PushTask(
                ...
                [&that] {
                    ...
                    if (!that.cfg.CreateChunk(chunkLoc)) {
                        ...
                        // chunk completed, we don't have to handle more than required. We close all the ones not
                        // working
