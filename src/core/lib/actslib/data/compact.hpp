...
namespace actslib::data::compact {
    ...
    class Bitmap64 {
      ...
      public:
        ...
        inline size_t WordOfId(size_t id) const { return id >> 3; }
        ...
        void UpdateCRC(actslib::crc::CRC32& crc) const { crc.Update(data, 0, datalen); }
    };
      ...
      public:
        ...
        uint64_t operator[](size_t id) const {
            ...
            if (wordOff + numbits > WORD_LEN) {
                // 2 words
                return (data[word] >> wordOff) |
                       ((data[word + 1] << ((WORD_LEN << 1) - wordOff - numbits)) >> (WORD_LEN - numbits));
            } else {
                // 1 word
        ...
        void UpdateCRC(actslib::crc::CRC32& crc) const { crc.Update(reinterpret_cast<char*>(data), 0, datalen); }
    };
