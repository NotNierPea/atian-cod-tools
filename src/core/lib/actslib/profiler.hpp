...
namespace actslib::profiler {
    ...
    class ProfilerSection {
      ...
      public:
         ...
         * @param name name
         */
        ProfilerSection(std::string name) : name(name) { endTime = startTime = GetTimestamp(); }
      ...
      public:
         ...
         * @param out stream
         */
        void WriteToStr(std::ostream& out) const { WriteToStr(out, 0); }

        constexpr const_iterator cbegin() const { return sections.cbegin(); }

        constexpr const_iterator cend() const { return sections.cend(); }

        constexpr iterator begin() { return sections.begin(); }

        constexpr iterator end() { return sections.end(); }

        constexpr const std::vector<ProfilerSection>& GetSubSections() const { return sections; }

        /*
         * @return is a sub section running in this section
         */
        constexpr bool IsRunning() const { return current != nullptr; }
        ...
        constexpr ProfTs GetMillis() const { return endTime - startTime; }

        constexpr ProfTs GetStartMillis() const { return startTime; }

        constexpr ProfTs GetEndMillis() const { return endTime; }
    ...
    class Profiler {
      ...
      public:
         ...
         * @return main section
         */
        const ProfilerSection& GetMainSection() const { return mainSec; }

        /*
         * @return current running section
         */
        ProfilerSection& GetCurrent() { return mainSec.GetCurrent(); }

        /*
         * Start new section
         * @param name section's name
         */
        void PushSection(std::string name) { mainSec.PushSection(name); }

        /*
         * End a section
         */
        void PopSection() { mainSec.PopSection(); }

        /*
         * End the profiler
         */
        void Stop() { mainSec.Stop(); }

        /*
         * Reset the profiler
         */
        void Reset() { mainSec = ProfilerSection{ name }; }
         ...
         * @param out stream
         */
        void WriteToStr(std::ostream& out) const { mainSec.WriteToStr(out); }
    };
    ...
    class ProfiledSection {
      ...
      public:
         ...
         * @param name section's name
         */
        ProfiledSection(Profiler& prof, std::string name) : prof(prof) { prof.PushSection(name); };
        ~ProfiledSection() { prof.PopSection(); }
    };
} // namespace actslib::profiler