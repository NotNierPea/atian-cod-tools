...
namespace actslib::crc {
    void CRC8::Update(const char val) { value = crc8Table[(value ^ val) & 0xFF]; }
    void CRC16::Update(const char val) { value = (crc16Table[(value ^ val) & 0xFF] ^ (value >> 8u)) & 0xFFFF; }
    void CRC32::Update(const char val) { value = crc32Table[(value ^ val) & 0xFF] ^ (value >> 8u); }
} // namespace actslib::crc
