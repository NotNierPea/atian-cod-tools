...
namespace actslib::rdf::raio {
    ...
    class CompressComponentReader : public data::iterator::AIterator<const IdComponent> {
      ...
      public:
        ...
        bool CheckEnd() { return io::DecodeVByte(is) == 4 && io::DecodeVByte(is) == 5 && io::DecodeVByte(is) == 6; }
...
#ifdef DEBUG
            if (!is)
                throw std::runtime_error("Invalid stream after compress read");
#endif
    ...
    class CompressComponentReaderFile : public CompressComponentReader {
      ...
      public:
        CompressComponentReaderFile(const std::filesystem::path& path)
            : stream(path, std::ios::binary), CompressComponentReader(stream) {
            if (!stream) {
...
template<>
struct std::formatter<actslib::rdf::raio::IdComponent, char>
    : actslib::BasicFormatter<actslib::rdf::raio::IdComponent> {};