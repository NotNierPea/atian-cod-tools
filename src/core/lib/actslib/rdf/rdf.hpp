...
namespace actslib::rdf {
    ...
    struct Component {
        ...
        char operator[](size_t idx) { return buffer[offset]; }
    ...
    class StringComponent : public Component {
      public:
        std::string strbuf;
        StringComponent(const Component& cmp)
            : strbuf{ cmp.buffer + cmp.offset, cmp.buffer + cmp.offset + cmp.length },
              Component{ nullptr, 0, cmp.length } {
            Component::buffer = strbuf.data();
        }
        StringComponent() : strbuf{}, Component{ nullptr, 0, 0 } { Component::buffer = strbuf.data(); }
    ...
    struct Triple {
        ...
        bool IsFull() const { return subject && predicate && object; }

        void Clear() { subject = predicate = object = nullptr; }
