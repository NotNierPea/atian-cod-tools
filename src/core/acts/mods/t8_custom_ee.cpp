...
int mods::ee::CustomEET8(Process& proc, std::string& notif) {
    ...
    TargetReplace targets[2] = { TargetReplace(hash::Hash64(".gsc", name), targetFunction, data, sizeof(data)),
    ...
    for (size_t i = 0; i < poolSize; i++) {
        for (auto& target : targets) {
            ...
            // find
            if (!proc.ReadMemory(&obj, buffer[i].buffer, sizeof(obj)) ||
                *(uint64_t*)obj.magic != tool::gsc::opcode::VMI_T8_36) {
            ...
            while (toRead > 0) {
                ...
                if (!proc.ReadMemory(&exports[0],
                                     buffer[i].buffer + obj.export_table_offset + shift * sizeof(exports[0]),
                                     read * sizeof(exports[0]))) {
