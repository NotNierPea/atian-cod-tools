    ...
    void LoadExtensionFile(AcefFileLoc* loc, bool preload) {
        ...
        try {
            ...
            for (size_t i = 0; i < header.numObjects; i++) {
                ...
                if (oloc != blockHeader.checkLoc)
                    throw std::runtime_error("invalid block location check");

                reader.Goto(blockHeader.checkLoc + header.sizeObjectHeader);

                if (blockHeader.blockSize > header.windowSize)
                    throw std::runtime_error("window too small");
                if (blockHeader.dataOffset + blockHeader.decompSize > header.decompressedSize)
                    throw std::runtime_error("too many data in block");
