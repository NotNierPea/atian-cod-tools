...
namespace hashutils {
    ...
    const std::unordered_map<uint64_t, const char*>& GetMap() { return core::hashes::GetMap(); }

    void* AllocHashMemory(size_t len) { return core::hashes::AllocHashMemory(len); }
    ...
    void ReadDefaultFile(bool cleanup) { ReadDefaultFileP(cleanup, nullptr); }
    ...
    void SetExtracted(std::unordered_set<uint64_t>* extracted) { g_extractedOut = extracted; }
    ...
    int LoadMap(const char* file, bool ignoreCol, bool iw, bool async) {
        ...
        // gsc temp vars
        static const char* gscTmpStart = "__a\0"  // array
                                         "__k\0"  // key
                                         "__ki\0" // key iterator
                                         "__ka\0" // keys array
            ;
...
void* ActsAPIHash_AllocHashMemory(size_t len) { return core::hashes::AllocHashMemory(len); }

const char* ActsAPIHash_CloneHashStr(const char* str) { return core::hashes::CloneHashStr(str); }

void ActsAPIHash_Clean() { core::hashes::Clean(); }

const char* ActsAPIHash_AddPrecomputed(uint64_t value, const char* str, bool clone) {
    return core::hashes::AddPrecomputed(value, str, clone);
}

const char* ActsAPIHash_ExtractPtr(uint64_t hash) { return core::hashes::ExtractPtr(hash); }
...
char* ActsAPIHash_ExtractTmp(const char* type, uint64_t hash) { return core::hashes::ExtractTmp(type, hash); }

void ActsAPIHash_ReadDefaultHashFiles() { ActsAPIHash_ReadDefaultHashFilesP(nullptr); }

void ActsAPIHash_ReadDefaultHashFilesP(ActsProgressHandler* progress) { hashutils::ReadDefaultFileP(false, progress); }
