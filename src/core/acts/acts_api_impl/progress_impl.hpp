...
class ActsProgressHandlerImpl {
  ...
  public:
    ...
    void Report(float progress, const std::string& message) { Report(progress, message.c_str()); }
    void Report(size_t count, size_t total, const std::string& message) { Report(count, total, message.c_str()); }
};