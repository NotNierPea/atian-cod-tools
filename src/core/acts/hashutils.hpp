...
namespace hashutils {
     ...
     * @param type Hash type
     * @param hash Hashed value
     * @return return value of extract and non thread-safe temporary pointer to a representation of this hash, the
     * result is valid until the next call to ExtractTmp
     */
     ...
     * @param type Hash type
     * @param hash Hashed value
     * @return non thread-safe temporary pointer to a representation of this hash, the result is valid until the next
     * call to ExtractTmp
     */
     ...
     * @return script formatted value, same condition as ExtractTmp
     */
    inline char* ExtractTmpScript(uint64_t hash) { return ExtractTmpPath("script", hash); }
    /*
