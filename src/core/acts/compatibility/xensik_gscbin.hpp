...
namespace compatibility::xensik::gscbin {
    ...
    struct GscBinHeader {
        ...
        constexpr byte* GetBuffer() { return magic + sizeof(GscBinHeader); }

        constexpr byte* GetByteCode() { return magic + sizeof(GscBinHeader) + compressedLen; }

        inline uint32_t& Magic() { return *reinterpret_cast<uint32_t*>(magic); }
    };
