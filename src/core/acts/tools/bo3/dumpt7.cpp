...
namespace {
    ...
    class PoolOption {
      public:
        ...
        bool HasFlag(uint64_t f) { return (flags & f) == f; }
    };
    ...
    if (ShouldHandle(T7_ASSET_TYPE_SCRIPTBUNDLE)) {
        ...
        for (size_t i = 0; i < entry.itemCount; i++) {
            ...
            if (p.sbObjectsArray.sbObjectCount) {
                ...
                auto [objs, oksb] =
                    proc.ReadMemoryArray<SB_Object>(p.sbObjectsArray.sbObjects, p.sbObjectsArray.sbObjectCount);
    ...
    int t7poolscripts(Process& proc, int argc, const char* argv[]) {
        ...
        for (size_t inst = 0; inst < 2; inst++) {
            ...
            for (size_t i = 0; i < gObjFileInfoCount[inst]; i++) {
                ...
                if (header.profile_offset < sizeof(header) ||
                    header.profile_offset < header.cseg_offset + header.cseg_size) {
        ...
        const uintptr_t spt =
            proc[xassetpools] + sizeof(bo3::pool::T7XAssetPool) * bo3::pool::T7_ASSET_TYPE_SCRIPTPARSETREE;
