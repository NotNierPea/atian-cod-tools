...
namespace tool::nui {
    ...
    bool tooluifunctiondata::operator!() const { return m_id == NULL; }

    bool tooluifunctiondata::operatorbool() const { return m_id != NULL; }

    namespace {
        std::default_random_engine re;

        void NuiSizeCallBack(GLFWwindow* wnd, int width, int height) { ActsNUI::nui->Resize(width, height); }
    } // namespace
    ...
    float& ActsConfig::Floor(size_t i, size_t j) { return floor[i * len + j]; }
    ...
    void ActsConfig::Render() {
        ...
        gluLookAt(5 * cameraX, 3, 5 * cameraY, 0, 0, 0, 0, 1, 0);
    ...
    ActsNUI::ActsNUI() {
        {
            ...
            if (nui)
                throw std::runtime_error("Can't register ActsNUI twice!");
    ...
    void ActsNUI::SetWindowName(const char* name) { glfwSetWindowTitle(wnd, name); }

    bool ActsNUI::ShouldClose() { return glfwWindowShouldClose(wnd); }
    ...
    int GetNUIScreenWidth() { return ActsNUI::nui->width; }

    int GetNUIScreenHeight() { return ActsNUI::nui->height; }
    ImVec2 GetDefaultWindowStart() { return ImVec2(ActsNUI::nui->bg.open ? 200.0f : 30.0f, 0); }
    ...
    void DisableNextBackground() { ActsNUI::nui->bg.nextDisabled = true; }

    void SaveNextConfig() { ActsNUI::nui->saveNext = true; }
