...
#pragma region radiantkeys
    ...
    const char* RadianKeysTypeName(RadianKeysType type) {
        ...
        case RKT_INTEGER:
            return "int";
        case RKT_HASH_LOCALIZED:
            return "localized";
        default:
            return utils::va("unk%d", type);
        }
    }
    ...
    int bo6_radiant_keys(int argc, const char* argv[]) {
        ...
        while (buffer.CanRead(sizeof(uint64_t))) {
            ...
            os << std::setw(10) << std::setfill(' ') << std::left << RadianKeysTypeName(type) << " "
               << hashutils::ExtractTmp("hash", key) << "\n";
        }
...
#pragma endregion
    ...
    int bo6_gsc_dump(int argc, const char* argv[]) {
        ...
        for (hook::library::ScanResult& res :
             mod->Scan("48 89 5C 24 08 55 48 8B EC 48 83 EC 40 48 8B D9 E8 ? ? ? ? ? ? ? ?")) {
        ...
        for (size_t i = 0; i < bo6GscData.pool.size(); i++) {
            ...
            for (size_t j = 0; j < pool.size(); j++) {
                ...
                for (size_t k = 0; k < container.data.count; k++) {
                    ...
                    os << "\n"
                       << std::dec << i << "," << j << "," << (container.isMethod ? "method" : "function") << ","
                       << hashutils::ExtractTmp("function", func.name) << "," << (int)func.minArgs << ","
                       << (int)func.maxArgs << "," << (func.devFunc ? "dev" : "common") << ","
    ...
    void Bo6LoadLuaPop(void* vm, size_t count) { bo6LuaData.currentFunc = defaultBo6LuaFuncObject; }
    void Bo6LoadLuaPush(void* vm, int a2, const char* category) { bo6LuaData.currentFunc = category; }

    void Bo6LoadLuaFunc(void* vm, uint64_t hash, void* func) { bo6LuaData.funcs[bo6LuaData.currentFunc][hash] = func; }
    ...
    int bo6_lua_dump(int argc, const char* argv[]) {
        ...
        }; // Bo6LoadLuaFunc(luastate*, 0xhash, func)
        void* stubStr{ mod->ScanSingle("E8 ? ? ? ? 4C 8D 05 ? ? ? ? 48 8D 15 ? ? ? ? 49 8B CC", "Bo6LoadLuaFuncStr")
                           .GetRelative<int32_t>(1) }; // Bo6LoadLuaFuncStr(luastate*, "str", func)

        LOG_INFO("redirect hash . {}", hook::library::CodePointer{ stub });
        LOG_INFO("redirect str .. {}", hook::library::CodePointer{ stubStr });

        hook::memory::RedirectJmp(stub, Bo6LoadLuaFunc);
    ...
    int bo6_test_hash_types(int argc, const char* argv[]) {
        ...
        for (size_t i = 0; i < bo6::T10R_ASSET_COUNT; i++) {
            os << std::dec << i << "," << bo6::PoolNameRelease((bo6::T10RAssetType)i) << ",0x" << std::hex
               << hash::HashX32(bo6::PoolNameRelease((bo6::T10RAssetType)i)) << "\n";
        }
    ...
    int bo6_asset_sizes(int argc, const char* argv[]) {
        ...
        for (size_t i = 0; i < bo6::T10RAssetType::T10R_ASSET_COUNT; i++) {
            *os << std::endl;
            ...
            os << std::dec << i << "," << bo6::PoolNameRelease(type) << ",0x" << nfo->itemSize << ","
               << (custom ? "true" : "false");
            ...
            os << "," << hook::library::CodePointer{ nfo->GetAssetName } << ","
    ...
    int bo6_hash_ffstr(int argc, const char* argv[]) {
        ...
        DB_HashScrStringName =
            mod->ScanSingle("48 89 5C 24 ? 57 48 83 EC ? 49 8B F8 4C 8B DA 48 8B D9 48", "gcx.DB_HashScrStringName")
