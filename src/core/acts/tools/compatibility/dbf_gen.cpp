...
namespace {
        ...
        pool::XAssetType GetAssetTypeFromPath(const std::filesystem::path& root, const std::filesystem::path& path,
uint64_t& nameout, bool& precompiled) { std::filesystem::path rel{ std::filesystem::relative(path, root) };
        ...
        int dbfgen(Process& proc, int argc, const char* argv[]) {
                ...
                for (const std::filesystem::path& path : paths) {
                        ...
                        LOG_INFO("Reading {} with type {} -> hash_{:x}", path.string(), pool::XAssetNameFromId(type),
name);
                        ...
                        case pool::ASSET_TYPE_STRINGTABLE: {
                                ...
                                for (size_t i = 0; i < doc.GetColumnCount(); i++) {
                                        ...
                                        if (cell == "undefined") {
                                                ...
                                                LOG_ERROR("Can't read {} StringTable type of column {} : '{}'",
path.string(), i, cell); err = true; break;
                                        }
                                ...
                                if (rows && columns) {
                                        // at least one cell
                                        auto [cellsId, cellsptr] =
builder.CreateBlock<StringTableCell>(sizeof(StringTableCell) * rows * columns);
                                        // link cells
                                        ...
                                        for (size_t row = 1; row <= rows; row++) {
                                                for (size_t column = 0; column < columns; column++) {
                                                        ...
                                                        const std::string cellVal = doc.GetCell<std::string>(column,
row);

                                                        StringTableCell& cell =
builder.GetBlock<StringTableCell>(cellsId)[idx]; cell.type = cellType;
                                                        ...
                                                        try {
                                                                ...
                                                                case STC_TYPE_UNDEFINED:
                                                                        ...
                                                                        break;
                                                                case STC_TYPE_BOOL:
                                                                        cell.value.bool_value = cellVal == "true" ||
cellVal == "TRUE"; break; case STC_TYPE_HASHED2: case STC_TYPE_HASHED7: case STC_TYPE_HASHED8: if (cellVal.empty()) {
                                                                                cell.value.hash_value = 0;
                                                                        }
                                                                        else {
                                                                                cell.value.hash_value =
hash::Hash64Pattern(cellVal.c_str());
                                                                        }
                                                                        break;
                                                                case STC_TYPE_INT:
                                                                        cell.value.int_value =
utils::ParseFormatInt(cellVal.c_str()); break; case STC_TYPE_FLOAT: cell.value.float_value = std::stof(cellVal); break;
                                                                case STC_TYPE_STRING: {
                                                                        dbflib::BlockId allocatedString =
builder.CreateBlock((void*)cellVal.data(), cellVal.size() + 1); builder.CreateLink(cellsId,
(dbflib::BlockOffset)(sizeof(StringTableCell) * idx + offsetof(StringTableCell, value)), allocatedString); break;
                                                                }
                                                                }
                                                        }
                                                        catch (const std::invalid_argument& e) {
                                                                LOG_WARNING("Can't read StringTable {} : {} [line {} col
{}] '{}'", path.string(), e.what(), row, column, cellVal); err = true; break;
                                                        }
                                                        idx++;
                                                }
                                        }
                        ...
                        case pool::ASSET_TYPE_LOCALIZE_ENTRY: {
                                ...
                                for (const auto& [k, v] : doc.GetObj()) {
                                        ...
                                        if (!v.IsString()) {
                                                LOG_ERROR("Invalid json for file {}, the key {} isn't a string",
path.string(), ks); err = true;
                                        }
                ...
                // write entries
                if (entries.size()) {
                        dbflib::BlockId entriesId = builder.CreateBlock(entries.data(), sizeof(entries[0]) *
entries.size());

                        for (size_t i = 0; i < entries.size(); i++) {
                                builder.CreateLink(entriesId, (dbflib::BlockOffset)(sizeof(entries[0]) * i +
offsetof(DBFFileAssetEntry, header)), entriesLoc[i]);
                        }
                ...
                // write ref entries
                if (refEntries.size()) {
                        dbflib::BlockId refEntriesId = builder.CreateBlock(refEntries.data(), sizeof(refEntries[0]) *
refEntries.size());

                        for (size_t i = 0; i < refEntries.size(); i++) {
                                builder.CreateLink(refEntriesId, (dbflib::BlockOffset)(sizeof(refEntries[0]) * i +
offsetof(DBFFileAssetAssetRef, location)), entriesLoc[i]);
                        }
                        builder.CreateLink(tableId, offsetof(DBFFileAssetTable, refEntries), refEntriesId);
                }

                if (refStrings.size()) {
                        dbflib::BlockId refStringsId = builder.CreateBlock(refStrings.data(), sizeof(refStrings[0]) *
refStrings.size());

                        for (size_t i = 0; i < refStrings.size(); i++) {
                                builder.CreateLink(refStringsId, (dbflib::BlockOffset)(sizeof(refStrings[0]) * i +
offsetof(DBFileAssetStringRef, str)), (dbflib::BlockId)refStrings[i].str); builder.CreateLink(refStringsId,
(dbflib::BlockOffset)(sizeof(refStrings[0]) * i + offsetof(DBFileAssetStringRef, location)),
(dbflib::BlockId)refStrings[i].location);
                        }
        ...
        int dbfread(Process& proc, int argc, const char* argv[]) {
                ...
                for (size_t i = 0; i < table->entriesCount; i++) {
                        ...
                        switch (e.type) {
                        case pool::ASSET_TYPE_LOCALIZE_ENTRY:
                                LOG_INFO("- {:x} ({}) -> '{}'", e.name, pool::XAssetNameFromId(e.type),
e.header.localize->string); break; case pool::ASSET_TYPE_SCRIPTPARSETREE: { std::filesystem::path outGscc{ outDir /
std::format("script_{:x}.gscc", e.header.spt->name.name) }; utils::WriteFile(outGscc, e.header.spt->buffer,
e.header.spt->size); LOG_INFO("- {:x} ({}) -> {}", e.name, pool::XAssetNameFromId(e.type), outGscc.string()); break;
                        }
                        default:
