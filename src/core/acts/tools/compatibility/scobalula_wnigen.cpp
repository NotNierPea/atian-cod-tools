    ...
    namespace {
        ...
        int wni_gen(int argc, const char* argv[]) {
            ...
            while (is.good() && std::getline(is, line)) {
                ...
                if (alg & ALG_SCR_T89)
                    i.insert(hash::HashT89Scr(cstr));
                if (alg & ALG_SCR_T7)
                    i.insert(hash::HashT7(cstr));
                if (alg & ALG_FNV32)
                    i.insert(hash::HashX32(cstr));
                if (alg & ALG_FNV)
                    i.insert(hash::Hash64(cstr));
                if (alg & ALG_FNV_IW_RES)
                    i.insert(hash::HashIWAsset(cstr));
                if (alg & ALG_SCR_JUP)
                    i.insert(hash::HashJupScr(cstr));
                if (alg & ALG_DVAR)
                    i.insert(hash::HashIWDVar(cstr));
                if (alg & ALG_SCR_T10)
                    i.insert(hash::HashT10Scr(cstr));
                if (alg & ALG_SCR_T10_SP)
                    i.insert(hash::HashT10ScrSP(cstr));
                if (alg & ALG_OMNVAR_T10)
                    i.insert(hash::HashT10OmnVar(cstr));
                count++;
            }
