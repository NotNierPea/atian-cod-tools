...
namespace tool::nui::renderer {
    ...
    void NuiFont::LoadFont(GLuint textureId, const char* str, size_t len, int w, int h) {
        ...
        if (configIt != doc.MemberEnd()) {
            ...
            if (charHeightIt != configIt->value.MemberEnd())
                fontOriginalHeight = charHeightIt->value.GetFloat() / h;
            if (sizeIt != configIt->value.MemberEnd())
                fontOriginalWidth = sizeIt->value.GetFloat() / w;
        }
        ...
        if (symbolsIt != doc.MemberEnd()) {
            if (symbolsIt->value.IsArray()) {
                ...
                for (const auto& symbol : arr) {
                    ...
                    if (idIt == symbol.MemberEnd())
                        continue; // no id, error

                    int id = idIt->value.GetInt();

                    if (id < minChar || id > maxChar)
                        continue; // bad id

                    NuiFontSymbolData& data{ symbolsData[id - minChar] };

                    if (xIt != symbol.MemberEnd())
                        data.x = xIt->value.GetFloat() / w;
                    if (yIt != symbol.MemberEnd())
                        data.y = yIt->value.GetFloat() / h;
                    if (xoffsetIt != symbol.MemberEnd())
                        data.offx = xoffsetIt->value.GetFloat() / w;
                    if (yoffsetIt != symbol.MemberEnd())
                        data.offy = yoffsetIt->value.GetFloat() / h;
                    if (heightIt != symbol.MemberEnd())
                        data.h = heightIt->value.GetFloat() / h;
                    if (widthIt != symbol.MemberEnd())
                        data.w = widthIt->value.GetFloat() / w;
