        ...
        class AssetListFFHandler : public FFHandler {
          public:
            ...
            void Handle(fastfile::FastFileOption& opt, core::bytebuffer::ByteBuffer& reader,
                ...
                std::filesystem::path outStrings{ opt.m_output / "assetlist" / "strings" / fftype /
                ...
                std::filesystem::path outPreAssets{ opt.m_output / "assetlist" / "pre_assets" / fftype /
                                                    std::format("{}.csv", ctx.ffname) };
                std::filesystem::path outAssets{ opt.m_output / "assetlist" / "assets" / fftype /
                                                 std::format("{}.csv", ctx.ffname) };
                if (gcx.list.assets) {
