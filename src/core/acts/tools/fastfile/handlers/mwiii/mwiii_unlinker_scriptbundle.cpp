...
namespace fastfile::handlers::mwiii::scriptbundle {
    ...
    bool WriteDef(HandlerJsonWriter& json, ScriptBundleObjectDef& def) {
        ...
        case SBT_ARRAY_INDEXED: {
            ...
            for (size_t i = 0; i < arrayIndexed->count; i++) {
                ...
                if (!WriteDef(json, arrayIndexed->vals[i]))
                    return false;
            }
            json.EndArray();
            break;
    ...
    bool WriteBundle(HandlerJsonWriter& json, ScriptBundle* bundle) { return WriteData(json, bundle->data); }
    ...
    utils::MapAdder<ImplWorker, HandlerHashedAssetType, Worker> impl{ GetWorkers(),
                                                                      HandlerHashedAssetType::JUPH_ASSET_SCRIPTBUNDLE,
                                                                      sizeof(ScriptBundle) };
} // namespace fastfile::handlers::mwiii::scriptbundle