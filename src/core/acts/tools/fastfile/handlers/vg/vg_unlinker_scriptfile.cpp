...
namespace {
    ...
    class ImplWorker : public Worker {
        ...
        void PreXFileLoading(fastfile::FastFileOption& opt, fastfile::FastFileContext& ctx) override { hashed.clear(); }
        void PostXFileLoading(fastfile::FastFileOption& opt, fastfile::FastFileContext& ctx) override {
            if (hashed.empty()) {
                return;
            }

            std::filesystem::path outFile{ opt.m_output / gamePath / "scriptfile" / "names" /
            ...
            for (ScriptFile* sf : hashed) {
                ...
                if (id) {
                    os << sf->name << ",0x" << std::hex << std::setw(4) << std::setfill('0') << id << ",#"
    ...
    utils::MapAdder<ImplWorker, HandlerHashedAssetType, Worker> impl{ GetWorkers(),
                                                                      HandlerHashedAssetType::IW8H_ASSET_SCRIPTFILE,
                                                                      sizeof(ScriptFile) };
} // namespace