...
namespace {
    ...
    class ImplWorker : public Worker {
        ...
        void Unlink(fastfile::FastFileOption& opt, fastfile::FastFileContext& ctx, void* ptr) override {
            ...
            if (asset->uf1)
                json.WriteFieldValueBool("uf1", asset->uf1);
            if (asset->uf2)
                json.WriteFieldValueBool("uf2", asset->uf2);
            if (asset->uf3)
                json.WriteFieldValueBool("uf3", asset->uf3);
            if (asset->uf4)
                json.WriteFieldValueBool("uf4", asset->uf4);
    ...
    class ImplSkinWorker : public Worker {
        ...
        void Unlink(fastfile::FastFileOption& opt, fastfile::FastFileContext& ctx, void* ptr) override {
            ...
            if (asset->unkb8)
                json.WriteFieldValueBool("unkb8", asset->unkb8);
            if (asset->unkb9)
                json.WriteFieldValueBool("unkb9", asset->unkb9);
            if (asset->unkba)
                json.WriteFieldValueBool("unkba", asset->unkba);
    ...
    utils::MapAdder<ImplSkinWorker, SatHashAssetType, Worker> impls{ GetWorkers(),
                                                                     ...
                                                                     sizeof(OperatorSkin) };
    utils::MapAdder<ImplReacWorker, SatHashAssetType, Worker> implr{ GetWorkers(),
                                                                     SatHashAssetType::SATH_ASSET_REACTIVEOPERATOR,
                                                                     sizeof(ReactiveOperator) };
    utils::MapAdder<ImplListWorker, SatHashAssetType, Worker> impll{ GetWorkers(),
                                                                     SatHashAssetType::SATH_ASSET_OPERATORLIST,
                                                                     sizeof(OperatorList) };
} // namespace