...
namespace {
    ...
    class ImplWorker : public Worker {
        ...
        void Unlink(fastfile::FastFileOption& opt, fastfile::FastFileContext& ctx, void* ptr) override {
            ...
            if (asset->dialog)
                json.WriteFieldValueString("dialog", asset->dialog);
            if (asset->gender)
                json.WriteFieldValueString("gender", asset->gender);

            // json.WriteFieldValueUnknown("unk18", asset->unk18); //-1?
            json.WriteFieldValueUnknown("unk20", asset->unk20); // unused?
            if (asset->unk8c)
                json.WriteFieldValueBool("unk8c", asset->unk8c);
            if (asset->unk8d)
                json.WriteFieldValueBool("unk8d", asset->unk8d);
            if (asset->unk8e)
                json.WriteFieldValueBool("unk8e", asset->unk8e);
            if (asset->unk8f)
                json.WriteFieldValueBool("unk8f", asset->unk8f);
            ...
            std::filesystem::path outFile{ opt.m_output / gamePath / "source" / "tables" / "operator" /
    ...
    class ImplSkinWorker : public Worker {
        ...
        void Unlink(fastfile::FastFileOption& opt, fastfile::FastFileContext& ctx, void* ptr) override {
            ...
            if (asset->unk88)
                json.WriteFieldValueBool("unk88", asset->unk88);
            if (asset->unk89)
                json.WriteFieldValueBool("unk89", asset->unk89);
            if (asset->unk8a)
                json.WriteFieldValueBool("unk8a", asset->unk8a);
            json.WriteFieldValueNumber("lootid", asset->lootid);
            ...
            if (asset->unk60)
                json.WriteFieldValueString("unk60", asset->unk60);

            json.WriteFieldNameString("bundle");
            scriptbundle::WriteData(json, asset->bundleData);
            json.EndObject();

            std::filesystem::path outFile{ opt.m_output / gamePath / "source" / "tables" / "operator" / "skin" /
                                           std::format("{}.json", hashutils::ExtractTmp("file", asset->name)) };
            std::filesystem::create_directories(outFile.parent_path());
    ...
    class ImplListWorker : public Worker {
        ...
        void Unlink(fastfile::FastFileOption& opt, fastfile::FastFileContext& ctx, void* ptr) override {
            ...
            std::filesystem::path outFile{ opt.m_output / gamePath / "source" / "tables" / "operator" / "list" /
                                           std::format("{}.json", hashutils::ExtractTmp("file", asset->name)) };
            std::filesystem::create_directories(outFile.parent_path());
    ...
    utils::MapAdder<ImplWorker, bo6::T10HashAssetType, Worker> impl{ GetWorkers(),
                                                                     bo6::T10HashAssetType::T10H_ASSET_OPERATOR,
                                                                     sizeof(Operator) };
    utils::MapAdder<ImplSkinWorker, bo6::T10HashAssetType, Worker> impls{
        GetWorkers(), bo6::T10HashAssetType::T10H_ASSET_OPERATORSKIN, sizeof(OperatorSkin)
    };
    utils::MapAdder<ImplReacWorker, bo6::T10HashAssetType, Worker> implr{
        GetWorkers(), bo6::T10HashAssetType::T10H_ASSET_REACTIVEOPERATOR, sizeof(ReactiveOperator)
    };
    utils::MapAdder<ImplListWorker, bo6::T10HashAssetType, Worker> impll{
        GetWorkers(), bo6::T10HashAssetType::T10H_ASSET_OPERATORLIST, sizeof(OperatorList)
    };
} // namespace