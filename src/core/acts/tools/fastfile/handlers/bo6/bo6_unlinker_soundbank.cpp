...
namespace {
    ...
    class ImplWorker : public Worker {
      ...
      public:
        ...
        void Unlink(fastfile::FastFileOption& opt, fastfile::FastFileContext& ctx, void* ptr) override {
            ...
            std::filesystem::path outFile{ opt.m_output / gamePath / "source" / "tables" / "sound" / type / ctx.ffname /
                ...
                if (asset->aliases) {
                    for (size_t i = 0; i < asset->count; i++) {
                        ...
                        if (aliases->aliases) {
                            for (size_t j = 0; j < aliases->count; j++) {
                                SndBankAlias* alias{ &aliases->aliases[j] };
                                if ((!alias->text->subtitles && !alias->text->character))
                                    continue; // invalid
    ...
    utils::MapAdder<ImplWorker, bo6::T10HashAssetType, Worker> impl{ GetWorkers(),
                                                                     bo6::T10HashAssetType::T10H_ASSET_SOUNDBANK,
                                                                     false };
    utils::MapAdder<ImplWorker, bo6::T10HashAssetType, Worker> implt{
        GetWorkers(), bo6::T10HashAssetType::T10H_ASSET_SOUNDBANKTRANSIENT, true
    };
} // namespace