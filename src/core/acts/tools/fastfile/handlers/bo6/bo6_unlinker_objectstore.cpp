...
namespace {
    ...
    class ImplWorkerGM : public Worker {
        ...
        void Unlink(fastfile::FastFileOption& opt, fastfile::FastFileContext& ctx, void* ptr) override {
            ...
            std::filesystem::path outFile{ opt.m_output / gamePath / "source" / "tables" / "objectstore" / "gamemode" /
                                           std::format("{}.json", hashutils::ExtractTmp("file", asset->name)) };
            std::filesystem::create_directories(outFile.parent_path());
            LOG_OPT_INFO("Dump objectstore project {}", outFile.string());
    ...
    utils::MapAdder<ImplWorker, bo6::T10HashAssetType, Worker> implp{
        GetWorkers(), bo6::T10HashAssetType::T10H_ASSET_OBJECTSTOREPROJECT, sizeof(ObjectStoreProject)
    };
    utils::MapAdder<ImplWorkerDDLCfg, bo6::T10HashAssetType, Worker> impldc{
        GetWorkers(), bo6::T10HashAssetType::T10H_ASSET_OBJECTSTOREDDLCONFIG, sizeof(ObjectStoreDDLConfig)
    };
    utils::MapAdder<ImplWorkerDDLCfgGrp, bo6::T10HashAssetType, Worker> impldcg{
        GetWorkers(), bo6::T10HashAssetType::T10H_ASSET_OBJECTSTOREDDLCONFIGGROUP, sizeof(ObjectStoreDDLConfigGroup)
    };
    utils::MapAdder<ImplWorkerGM, bo6::T10HashAssetType, Worker> implg{
        GetWorkers(), bo6::T10HashAssetType::T10H_ASSET_OBJECTSTOREGAMEMODE, sizeof(ObjectStoreGamemode)
    };
} // namespace