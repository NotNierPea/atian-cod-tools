...
namespace {
    ...
    const char* LookupLocalizedImpl(uint64_t hash) { return fastfile::GetCurrentOptions().GetTranslation(hash); }
    ...
    class ImplWorker : public Worker {
        ...
        void PreLoadWorker(fastfile::FastFileContext* ctx) override {
            ...
            const char* names[][2]{ { "", "" },    { "", "_fx" },   { "", "_lighting" }, { "", "_precache" },
                                    { "", "_ob" }, { "", "_code" }, { "", "_audio" },    { "gen/", "_art" } };
        ...
        void Unlink(fastfile::FastFileOption& opt, fastfile::FastFileContext& ctx, void* ptr) override {
            ...
            std::filesystem::path outFile{ opt.m_output / gamePath / "gscobj" /
                                           std::format("script_{:x}.gscc", asset->name) };
    ...
    class ImplWorkerGdb : public Worker {
        ...
        void Unlink(fastfile::FastFileOption& opt, fastfile::FastFileContext& ctx, void* ptr) override {
            ...
            std::filesystem::path outFile{ opt.m_output / gamePath / "gscobj" /
                                           std::format("script_{:x}.gsc.gdb", asset->name) };
    ...
    utils::MapAdder<ImplWorker, HandlerHashedAssetType, Worker> impl{ GetWorkers(),
                                                                      HandlerHashedAssetType::JUPH_ASSET_GSCOBJ,
                                                                      sizeof(GscObj) };
    utils::MapAdder<ImplWorkerGdb, HandlerHashedAssetType, Worker> implGdb{ GetWorkers(),
                                                                            HandlerHashedAssetType::JUPH_ASSET_GSCGDB,
                                                                            sizeof(GscGdb) };
} // namespace