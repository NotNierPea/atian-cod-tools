...
namespace {
    ...
    class BgCacheWorker : public Worker {
        void Unlink(fastfile::FastFileOption& opt, void* ptr) {
            ...
            std::filesystem::path outFile{ opt.m_output / "bo4" / "source" / "tables" / "bgcache" /
                                           std::format("{}.csv", n) };
            ...
            for (size_t i = 0; i < asset->defCount; i++) {
                ...
                os << "\n"
                   << games::bo4::pool::BGCacheNameFromId(def.type) << ",#"
                   << hashutils::ExtractTmp("hash", def.name.name);
            }
        }
    };

    utils::MapAdder<BgCacheWorker, games::bo4::pool::XAssetType, Worker> impl{
        GetWorkers(), games::bo4::pool::XAssetType::ASSET_TYPE_BG_CACHE
    };
} // namespace