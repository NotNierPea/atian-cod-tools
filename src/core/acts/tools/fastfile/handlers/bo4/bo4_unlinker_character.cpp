...
namespace {
    ...
    const char* characterVoiceNames[]{
        "american", "russian", "unita",     "cuban", "mujahideen",    "pdf", "pmc",     "isi",
        "digbat",   "yemeni",  "terrorist", "lapd",  "secretservice", "mp",  "chinese",
    };
    ...
    class ImplWorker : public Worker {
        void Unlink(fastfile::FastFileOption& opt, void* ptr) {
            ...
            if (asset->chestGear_Right_Required)
                json.WriteFieldValueBool("chestGear_Right_Required", asset->chestGear_Right_Required);
            json.WriteFieldValueXAsset("chestGear_Left", ASSET_TYPE_XMODEL, asset->chestGear_Left);
            if (asset->chestGear_Left_Required)
                json.WriteFieldValueBool("chestGear_Left_Required", asset->chestGear_Left_Required);
            json.WriteFieldValueXAsset("frontPack", ASSET_TYPE_XMODEL, asset->frontPack);
            if (asset->frontPack_Required)
                json.WriteFieldValueBool("frontPack_Required", asset->frontPack_Required);
            json.WriteFieldValueXAsset("frontPackLow", ASSET_TYPE_XMODEL, asset->frontPackLow);
            if (asset->frontPackLow_Required)
                json.WriteFieldValueBool("frontPackLow_Required", asset->frontPackLow_Required);
            json.WriteFieldValueXAsset("backPack", ASSET_TYPE_XMODEL, asset->backPack);
            if (asset->backPack_Required)
                json.WriteFieldValueBool("backPack_Required", asset->backPack_Required);
            json.WriteFieldValueXAsset("rearGear_Right", ASSET_TYPE_XMODEL, asset->rearGear_Right);
            if (asset->rearGear_Right_Required)
                json.WriteFieldValueBool("rearGear_Right_Required", asset->rearGear_Right_Required);
            json.WriteFieldValueXAsset("rearGear_Left", ASSET_TYPE_XMODEL, asset->rearGear_Left);
            if (asset->rearGear_Left_Required)
                json.WriteFieldValueBool("rearGear_Left_Required", asset->rearGear_Left_Required);
            json.WriteFieldValueXAsset("rearGearSide_Right", ASSET_TYPE_XMODEL, asset->rearGearSide_Right);
            if (asset->rearGearSide_Right_Required)
                json.WriteFieldValueBool("rearGearSide_Right_Required", asset->rearGearSide_Right_Required);
            json.WriteFieldValueXAsset("rearGearSide_Left", ASSET_TYPE_XMODEL, asset->rearGearSide_Left);
            if (asset->rearGearSide_Left_Required)
                json.WriteFieldValueBool("rearGearSide_Left_Required", asset->rearGearSide_Left_Required);
            json.WriteFieldValueXAsset("sideGear_Right", ASSET_TYPE_XMODEL, asset->sideGear_Right);
            if (asset->sideGear_Right_Required)
                json.WriteFieldValueBool("sideGear_Right_Required", asset->sideGear_Right_Required);
            json.WriteFieldValueXAsset("sideGear_Left", ASSET_TYPE_XMODEL, asset->sideGear_Left);
            if (asset->sideGear_Left_Required)
                json.WriteFieldValueBool("sideGear_Left_Required", asset->sideGear_Left_Required);
            json.WriteFieldValueXAsset("frontGear_Right", ASSET_TYPE_XMODEL, asset->frontGear_Right);
            if (asset->frontGear_Right_Required)
                json.WriteFieldValueBool("frontGear_Right_Required", asset->frontGear_Right_Required);
            json.WriteFieldValueXAsset("frontGear_Left", ASSET_TYPE_XMODEL, asset->frontGear_Left);
            if (asset->frontGear_Left_Required)
                json.WriteFieldValueBool("frontGear_Left_Required", asset->frontGear_Left_Required);
            json.WriteFieldValueXAsset("helmet", ASSET_TYPE_XMODEL, asset->helmet);
            if (asset->helmet_Required)
                json.WriteFieldValueBool("helmet_Required", asset->helmet_Required);
            json.WriteFieldValueXAsset("misc1", ASSET_TYPE_XMODEL, asset->misc1);
            if (asset->misc1_Required)
                json.WriteFieldValueBool("misc1_Required", asset->misc1_Required);
            json.WriteFieldValueXAsset("misc2", ASSET_TYPE_XMODEL, asset->misc2);
            if (asset->misc2_Required)
                json.WriteFieldValueBool("misc2_Required", asset->misc2_Required);
            json.WriteFieldValueXAsset("misc3", ASSET_TYPE_XMODEL, asset->misc3);
            if (asset->misc3_Required)
                json.WriteFieldValueBool("misc3_Required", asset->misc3_Required);
    ...
    utils::MapAdder<ImplWorker, games::bo4::pool::XAssetType, Worker> impl{
        GetWorkers(), games::bo4::pool::XAssetType::ASSET_TYPE_CHARACTER
    };
} // namespace