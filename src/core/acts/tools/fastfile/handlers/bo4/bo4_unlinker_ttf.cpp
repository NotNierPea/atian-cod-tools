...
namespace {
    ...
    class ImplWorker : public Worker {
        ...
        void Unlink(fastfile::FastFileOption& opt, void* ptr) {
            ...
            std::filesystem::path outFileGraphic{ opt.m_output / "bo4" / "graphics" / "ttf" /
                                                  std::format("{}.ttf", hashutils::ExtractTmp("file", asset->name)) };
            std::filesystem::path outFile{ opt.m_output / "bo4" / "source" / "tables" / "ttf" /
            ...
            if (asset->next)
                json.WriteFieldValueXHash("next", asset->next->name);
    ...
    utils::MapAdder<ImplWorker, games::bo4::pool::XAssetType, Worker> impl{
        GetWorkers(), games::bo4::pool::XAssetType::ASSET_TYPE_TTF
    };
} // namespace