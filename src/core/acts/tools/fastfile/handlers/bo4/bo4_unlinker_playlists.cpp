...
namespace {
    ...
    static const char* PlaylistRuleTypesNames[]{ "GAMETYPE_SETTINGS", "SET_DVAR", "EXEC_COMMAND" };
    ...
    void WritePlayListRule(BO4JsonWriter& json, PlaylistRule* rule) {
        ...
        if (rule->value)
            json.WriteFieldValueString("value", rule->value);
        ...
        if (rule->unk30_count) {
            ...
            for (size_t k = 0; k < rule->unk30_count; k++) {
                ...
                if (v->unk10)
                    json.WriteFieldValueString("unk10", v->unk10);
                json.EndObject();
            }
            json.EndArray();
        }
    ...
    class ImplWorker : public Worker {
        ...
        void Unlink(fastfile::FastFileOption& opt, void* ptr) {
            ...
            std::filesystem::path outDir{ opt.m_output / "bo4" / "source" / "tables" / "playlists" /
                                          fastfile::GetCurrentContext().ffname /
                                          hashutils::ExtractTmp("file", asset->name) };
            ...
            for (size_t i = 0; i < asset->categories_count; i++) {
                ...
                std::filesystem::path outFile{ outDir / "categories" /
                                               std::format("{}.json", hashutils::ExtractTmp("file", group->name)) };
                ...
                for (size_t j = 0; j < group->entries_count; j++) {
                    ...
                    if (entry->dlcName)
                        json.WriteFieldValueString("dlcName", entry->dlcName);
                    ...
                    // unk
                    if (entry->unk111)
                        json.WriteFieldValueNumber("unk111", (int)entry->unk111);
                    if (entry->unk112)
                        json.WriteFieldValueNumber("unk112", entry->unk112);
                    if (entry->unk114)
                        json.WriteFieldValueNumber("unk114", entry->unk114);
                    // unused?
                    if (entry->unkbc)
                        json.WriteFieldValueNumber("unkbc", entry->unkbc);
                    if (entry->unkfc)
                        json.WriteFieldValueNumber("unkfc", entry->unkfc);
                    if (entry->unkc0)
                        json.WriteFieldValueXHash("unkc0", entry->unkc0);
                    if (entry->unkb4)
                        json.WriteFieldValueBool("unkb4", entry->unkb4);
...
*/
                    ...
                    if (entry->unke0_count) {
                        ...
                        for (size_t k = 0; k < entry->unke0_count; k++) {
                            ...
                            if (unke0->name)
                                json.WriteFieldValueString("name", unke0->name);

                            if (unke0->unk0)
                                json.WriteFieldValueNumber("unk0", unke0->unk0);
                            if (unke0->unk4)
                                json.WriteFieldValueNumber("unk4", unke0->unk4);
                            if (unke0->unk8)
                                json.WriteFieldValueNumber("unk8", unke0->unk8);
                    ...
                    for (size_t k = 0; k < entry->rotations_count; k++) {
                        ...
                        if (rotation->map)
                            json.WriteFieldValueXHash("map", rotation->map->name);
                        if (rotation->gametype)
                            json.WriteFieldValueXHash("gametype", rotation->gametype->name);
    ...
    class PlaylistsGlobalSettingsImplWorker : public Worker {
        ...
        void Unlink(fastfile::FastFileOption& opt, void* ptr) {
            ...
            std::filesystem::path outFile{ opt.m_output / "bo4" / "source" / "tables" / "playlists" /
                                           fastfile::GetCurrentContext().ffname /
    ...
    utils::MapAdder<ImplWorker, games::bo4::pool::XAssetType, Worker> impl{
        GetWorkers(), games::bo4::pool::XAssetType::ASSET_TYPE_PLAYLISTS
    };
    utils::MapAdder<PlaylistsGlobalSettingsImplWorker, games::bo4::pool::XAssetType, Worker> implgs{
        GetWorkers(), games::bo4::pool::XAssetType::ASSET_TYPE_PLAYLIST_GLOBAL_SETTINGS
    };
    utils::MapAdder<PlaylistEventScheduleEntryImplWorker, games::bo4::pool::XAssetType, Worker> impls{
        GetWorkers(), games::bo4::pool::XAssetType::ASSET_TYPE_PLAYLIST_SCHEDULE
    };
} // namespace