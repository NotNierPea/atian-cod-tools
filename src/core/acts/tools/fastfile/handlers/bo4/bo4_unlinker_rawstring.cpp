...
namespace {
    ...
    class RawStringWorker : public Worker {
        ...
        void Unlink(fastfile::FastFileOption& opt, void* ptr) override { assets.push_back((RawString*)ptr); }
        void PreXFileLoading(fastfile::FastFileOption& opt, fastfile::FastFileContext& ctx) override { assets.clear(); }
        void PostXFileLoading(fastfile::FastFileOption& opt, fastfile::FastFileContext& ctx) override {
            if (assets.empty())
                return; // nothing to dump

            std::filesystem::path outFile{ opt.m_output / "bo4" / "source" / "tables" / "rawstring" /
    ...
    utils::MapAdder<RawStringWorker, games::bo4::pool::XAssetType, Worker> impl{
        GetWorkers(), games::bo4::pool::XAssetType::ASSET_TYPE_RAWSTRING
    };
} // namespace