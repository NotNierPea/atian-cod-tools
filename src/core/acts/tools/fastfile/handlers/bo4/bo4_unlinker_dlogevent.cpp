...
namespace {
    ...
    const char* DLogEventUnkFieldMemberTypeNames[]{
        "INVALID", "BOOL",   "FLOAT",  "INT8",   "INT16",  "INT32", "INT64",
        "UINT8",   "UINT16", "UINT32", "UINT64", "STRING", "UNK12", "XHASH",
    };
    ...
    class ImplWorker : public Worker {
        void Unlink(fastfile::FastFileOption& opt, void* ptr) {
            ...
            std::filesystem::path outFile{ opt.m_output / "bo4" / "source" / "tables" / "dlogevent" /
                                           std::format("{}.json", n) };
            ...
            auto WriteMember = [&WriteType, &json](DLogEventFieldMember* member) {
                ...
                if (member->len)
                    json.WriteFieldValueNumber("len", member->len);
                if (member->unk20)
                    json.WriteFieldValueBool("s111u20", member->unk20);
                json.EndObject();
            };
            ...
            if (asset->fields) {
                ...
                for (size_t i = 0; i < asset->numFields; i++) {
                    ...
                    if (sub->members) {
                        ...
                        if (sub11->unk10)
                            json.WriteFieldValueString("s11unk10", sub11->unk10);
                        if (sub11->members) {
                            json.WriteFieldNameString("members");
                            json.BeginArray();
    ...
    struct DLogEventUnk3
    {
            ...
            DLogEventUnk31* unk8;
                    ...
                    if (sub->unk0)
                        json.WriteFieldValueHash("s3u0", sub->unk0);
                    if (sub->unk4)
                        json.WriteFieldValueHash("s3u4", sub->unk4);
                    if (sub->unk10)
                        json.WriteFieldValueHash("s3u10", sub->unk10);
                    if (sub->unk14)
                        json.WriteFieldValueHash("s3u14", sub->unk14);
                    json.WriteFieldValueNumber("s3u18", sub->unk18);
                    json.WriteFieldValueNumber("s3u1c", sub->unk1c);
                    if (sub->unk20)
                        json.WriteFieldValueHash("s3u20", sub->unk20);
                    ...
                    if (sub->unk8) {
                        ...
                        if (sub31->unk10)
                            json.WriteFieldValueHash("s3u10", sub31->unk10);
                        if (sub31->unk18)
                            json.WriteFieldValueHash("s3u18", sub31->unk18);
                        if (sub31->unk1c)
                            json.WriteFieldValueHash("s3u1c", sub31->unk1c);
                        if (sub31->unk20)
                            json.WriteFieldValueBool("s3u20", sub31->unk20);
                        if (sub31->unk28) {
    ...
    utils::MapAdder<ImplWorker, games::bo4::pool::XAssetType, Worker> impl{
        GetWorkers(), games::bo4::pool::XAssetType::ASSET_TYPE_DLOGEVENT
    };
} // namespace