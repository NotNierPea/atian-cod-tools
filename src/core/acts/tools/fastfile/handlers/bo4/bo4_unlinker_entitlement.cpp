...
namespace {
    ...
    class EntitlementWorker : public Worker {
        void Unlink(fastfile::FastFileOption& opt, void* ptr) {
            ...
            std::filesystem::path outFile{ opt.m_output / "bo4" / "source" / "tables" / "entitlement" /
            ...
            if (asset->hasFirstPartyEntitlement)
                json.WriteFieldValueBool("hasFirstPartyEntitlement", asset->hasFirstPartyEntitlement);
            if (asset->unk11)
                json.WriteFieldValueBool("unk11", asset->unk11);
            if (asset->unk12)
                json.WriteFieldValueBool("unk12", asset->unk12);
            if (asset->hasItemIds)
                json.WriteFieldValueBool("hasItemIds", asset->hasItemIds);
    ...
    class EntitlementListWorker : public Worker {
        void Unlink(fastfile::FastFileOption& opt, void* ptr) {
            ...
            std::filesystem::path outFile{ opt.m_output / "bo4" / "source" / "tables" / "entitlement" / "list" /
    ...
    class FirstPartyEntitlementWorker : public Worker {
        void Unlink(fastfile::FastFileOption& opt, void* ptr) {
            ...
            std::filesystem::path outFile{ opt.m_output / "bo4" / "source" / "tables" / "entitlement" / "firstparty" /
                                           std::format("{}.json", hashutils::ExtractTmp("file", asset->name)) };
    ...
    class FirstPartyEntitlementListWorker : public Worker {
        void Unlink(fastfile::FastFileOption& opt, void* ptr) {
            ...
            std::filesystem::path outFile{ opt.m_output / "bo4" / "source" / "tables" / "entitlement" / "firstparty" /
                                           "list" / std::format("{}.csv", hashutils::ExtractTmp("file", asset->name)) };
    ...
    utils::MapAdder<EntitlementWorker, games::bo4::pool::XAssetType, Worker> imple{
        GetWorkers(), games::bo4::pool::XAssetType::ASSET_TYPE_ENTITLEMENT
    };
    utils::MapAdder<EntitlementListWorker, games::bo4::pool::XAssetType, Worker> implel{
        GetWorkers(), games::bo4::pool::XAssetType::ASSET_TYPE_ENTITLEMENTLIST
    };
    utils::MapAdder<FirstPartyEntitlementWorker, games::bo4::pool::XAssetType, Worker> implfpe{
        GetWorkers(), games::bo4::pool::XAssetType::ASSET_TYPE_FIRSTPARTYENTITLEMENT
    };
    utils::MapAdder<FirstPartyEntitlementListWorker, games::bo4::pool::XAssetType, Worker> implfpel{
        GetWorkers(), games::bo4::pool::XAssetType::ASSET_TYPE_FIRSTPARTYENTITLEMENTLIST
    };
} // namespace