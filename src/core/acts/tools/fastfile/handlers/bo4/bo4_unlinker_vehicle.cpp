...
namespace {
    ...
    const char* VehicleTypesNames[]{
        "wheels4", "motorcycle", "tank", "plane", "boat", "artillery", "helicopter",
    };

    const char* VehicleTypesName(VehicleTypes type) {
        if (type >= ACTS_ARRAYSIZE(VehicleTypesNames))
            return utils::va("<invalid:%d>", type);
    ...
    class VehicleDefWorker : public Worker {
        void Unlink(fastfile::FastFileOption& opt, void* ptr) {
            ...
            std::filesystem::path outFile{ opt.m_output / "bo4" / "source" / "tables" / "vehicle" /
                                           std::format("{}.json", n) };
            ...
            json.WriteValueNumber(asset->skidSpeedMax);
            json.WriteFieldNameString("peelSpeedMax");
            json.WriteValueNumber(asset->peelSpeedMax);
    ...
    utils::MapAdder<VehicleDefWorker, games::bo4::pool::XAssetType, Worker> impl{
        GetWorkers(), games::bo4::pool::XAssetType::ASSET_TYPE_VEHICLEDEF
    };
} // namespace