...
namespace {
    ...
    class ImplWorker : public Worker {
        ...
        void Unlink(fastfile::FastFileOption& opt, void* ptr) {
            ...
            std::filesystem::path outFile{ opt.m_output / "bo4" / "source" / "tables" / "talent" /
            ...
            json.EndArray();
            if (asset->unkf4)
                json.WriteFieldValueNumber("unkf4", asset->unkf4);
            if (asset->unkf8)
                json.WriteFieldValueNumber("unkf8", asset->unkf8);
            if (asset->unkfc)
                json.WriteFieldValueNumber("unkfc", asset->unkfc);
            if (asset->unk100)
                json.WriteFieldValueNumber("unk100", asset->unk100);
            if (asset->unk104)
                json.WriteFieldValueNumber("unk104", asset->unk104);
            if (asset->unk108)
                json.WriteFieldValueNumber("unk108", asset->unk108);
            if (asset->unk10c)
                json.WriteFieldValueNumber("unk10c", asset->unk10c);
    ...
    utils::MapAdder<ImplWorker, games::bo4::pool::XAssetType, Worker> impl{
        GetWorkers(), games::bo4::pool::XAssetType::ASSET_TYPE_TALENT, true
    };
} // namespace