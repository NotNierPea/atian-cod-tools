...
namespace {
    ...
    class ImplWorker : public Worker {
        ...
        void Unlink(fastfile::FastFileOption& opt, void* ptr) {
            ...
            {
                std::filesystem::path outFile{ opt.m_output / "bo4" / "source" / "tables" / "gametype" /
                                               std::format("{}.json", name) };
                BO4JsonWriter json{};
                json.BeginObject();
            ...
            for (size_t i = 0; i < asset->gameTypeCount; i++) {
                ...
                if (entry->nameStr)
                    json.WriteFieldValueString("nameStr", entry->nameStr);
                if (entry->unk8)
                    json.WriteFieldValueString("unk8", entry->unk8);
                if (entry->baseGameType)
                    json.WriteFieldValueString("baseGameType", entry->baseGameType);
                if (entry->groupName)
                    json.WriteFieldValueString("groupName", entry->groupName);
                if (entry->scoreInfoFile)
                    json.WriteFieldValueString("scoreInfoFile", entry->scoreInfoFile);
                ...
                if (entry->unka8)
                    json.WriteFieldValueNumber("unka8", entry->unka8);
                if (entry->unkac)
                    json.WriteFieldValueNumber("unkac", entry->unkac);
                ...
                if (entry->settingsCount) {
                    ...
                    for (size_t j = 0; j < entry->settingsCount; j++) {
                        ...
                        if (def->name)
                            json.WriteFieldValueString("nameStr", def->name);
                        if (def->value)
                            json.WriteFieldValueString("value", def->value);
    ...
    utils::MapAdder<ImplWorker, games::bo4::pool::XAssetType, Worker> impl{
        GetWorkers(), games::bo4::pool::XAssetType::ASSET_TYPE_GAMETYPETABLE
    };
} // namespace