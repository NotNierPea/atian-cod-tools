...
namespace {
    ...
    class StreamKeyWorker : public Worker {
        ...
        void Unlink(fastfile::FastFileOption& opt, void* ptr) override { keys.push_back((StreamKey*)ptr); }
        ...
        void PostXFileLoading(fastfile::FastFileOption& opt, fastfile::FastFileContext& ctx) override {
            ...
            std::filesystem::path outFile{ opt.m_output / "bo4" / "source" / "tables" / "streamkey" /
            ...
            for (StreamKey* key : keys) {
                ...
                if (key->unk38)
                    json.WriteFieldValueNumber("unk38", key->unk38);
                if (key->unk44)
                    json.WriteFieldValueNumber("unk44", key->unk44);
                if (key->unk46)
                    json.WriteFieldValueNumber("unk46", (int)key->unk46);
    ...
    utils::MapAdder<StreamKeyWorker, games::bo4::pool::XAssetType, Worker> impl{
        GetWorkers(), games::bo4::pool::XAssetType::ASSET_TYPE_STREAMKEY
    };
} // namespace