      ...
      public:
        ...
        void Unlink(fastfile::FastFileOption& opt, void* ptr) {
            ...
            if (name) {
                ...
                outFile = opt.m_output / "cw" / "source" / "tables" / "usings" / type / "hashed" /
                          std::format("file_{:x}.{}.json", asset->name, ext);
            }
            ...
            if (asset->serverName) {
                ...
                json.WriteValueHash(asset->serverName);
            }
            if (asset->clientName) {
                json.WriteFieldNameString("clientName");
                json.WriteValueHash(asset->clientName);
            }
            json.EndObject();
