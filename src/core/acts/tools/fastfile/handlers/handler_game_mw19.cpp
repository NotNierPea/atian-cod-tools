...
#define ThrowFastFileError(...) ThrowFastFileError_(std::format(__VA_ARGS__))
        ...
        uint32_t DB_GetAssetSize(HandlerAssetType type) { return gcx.assetSizes[type]; }
        ...
        class FFHandlerImpl : public fastfile::FFHandler {
          public:
            FFHandlerImpl()
                : fastfile::FFHandler(handlerId, handlerName, compatibility::scobalula::csi::CordycepGame::CG_MW4) {
                forceNumXBlocks = 8;
            }
