...
namespace fastfile::handlers::bo6sp {
    ...
    namespace {
        ...
        bool LoadStream(bool atStreamStart, void* ptr, int64_t len) {
            LOG_TRACE("LoadStream({}, {}, 0x{:x}/0x{:x}) {}", atStreamStart, ptr, len, gcx.reader->Remaining(),
                      hook::library::CodePointer{ _ReturnAddress() });
        ...
        void Load_String(char** pstr) {
            LOG_TRACE("Load_String({}/0x{:x}:0x{:x}) {}", (void*)pstr, gcx.reader->Loc(), gcx.reader->Remaining(),
                      hook::library::CodePointer{ _ReturnAddress() });
            char* str{ *pstr };
            do {
        ...
        class BO6FFHandler : public fastfile::FFHandler {
          public:
            ...
            BO6FFHandler()
                : fastfile::FFHandler("bo6sp", "Black Ops 6 Campaign",
                                      compatibility::scobalula::csi::CordycepGame::CG_BO6) {
                gcx.handler = this;
            }
            ...
            void Handle(fastfile::FastFileOption& opt, core::bytebuffer::ByteBuffer& ffData,
                ...
                std::filesystem::path outStrings{ gcx.opt->m_output / "bo6sp" / "source" / "tables" / "data" /
...

                {
                    ...
                    for (size_t i = 0; i < gcx.assets.stringsCount; i++) {
                        ...
                        if (stroff) {
                            if (stroff & StreamPointerFlag::SPF_NEXT) {
                                if (stroff & StreamPointerFlag::SPF_CREATE_REF) {
                                    stringsOs << "[ref:" << std::setw(16) << std::setfill('0') << std::hex
                                              << (stroff & StreamPointerFlag::SPF_DATA_MASK) << "]";
                                } else {
                                    stringsOs << "[next]";
                                }
                            } else {
                                stringsOs << "[off:" << std::setw(16) << std::setfill('0') << std::hex
                                          << (stroff & StreamPointerFlag::SPF_DATA_MASK) << "]";
                            }
                        } else {
                            stringsOs << "[null]";
                ...
                std::filesystem::path outAssets{ gcx.opt->m_output / "bo6sp" / "source" / "tables" / "data" / "assets" /
                                                 fftype / std::format("{}.csv", ctx.ffname) };
                {
                    ...
                    try {
                        for (assetId = 0; assetId < gcx.assets.assetsCount; assetId++) {
                            ...
                            LOG_DEBUG("load #{} -> {}({})", assetId, PoolName(bo6sp::GetHashType(asset->type)),
                                      (int)asset->type);
                ...
                if (gcx.xstringLocs) {
                    std::filesystem::path ostr{ gcx.opt->m_output / "bo6sp" / "source" / "tables" / "data" /
    ...
    const char* GetPoolName(uint32_t hash) { return gcx.assetNames.GetMappedType((T10HashAssetType)hash)->name; }
    ...
    T10HashAssetType GetHashType(T10AssetType type) { return gcx.assetNames.GetHashType(type); }

    T10AssetType GetExePoolId(const char* name) { return gcx.assetNames.GetExePoolId(name); }
    T10AssetType GetExePoolId(T10HashAssetType name) { return gcx.assetNames.GetExePoolId(name); }
