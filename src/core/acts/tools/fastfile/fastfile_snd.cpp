...
namespace {
    ...
    const char* SndAssetFormatNames[]{
        "PCMS16", "PCMS24", "PCMS32", "IEEE", "XMA4", "MP3", "MSADPCM", "WMA", "FLAC", "WIIUADPCM", "MPC",
    };
    ...
    const char* SndAssetFormatName(SndAssetFormat format) {
        if (format < ACTS_ARRAYSIZE(SndAssetFormatNames))
            return SndAssetFormatNames[format];
        return utils::va("Invalid:%d", (int)format);
    }

    const char* SndAssetFormatExt(SndAssetFormat format) {
        if (format < ACTS_ARRAYSIZE(SndAssetFormatExts))
            return SndAssetFormatExts[format];
        return ".bin";
    }
    ...
    int ffsndr(int argc, const char* argv[]) {
        ...
        // AssetNameOffset seem to contain the asset hashed names with a 0x78 padding (0x80), probably remnant from a
        // char name[0x80] SourceChecksumOffset and checksumOffset are 128 bits values for each entries (checksumSize?)
        ...
        if (header.entryCount) {
            ...
            for (size_t i = 0; i < header.entryCount; i++) {
                ...
                if (!name)
                    name = utils::va("hashed/snd/%s/file_%llx", lang[0] ? lang : "all", entry.id);

                std::filesystem::path of{ std::filesystem::path{ "output_ff/snd/" } /
