    ...
    class XAssetLinkerImpl : public XAssetLinker {
      public:
        ...
        void Compute(BO4LinkContext& ctx, const char* id, BO4FFContext& ff) override {
            ...
            zbarrier.upgradedGeneralRepairSound1.name =
                ctx.HashXHash(objCfg.GetCString("upgradedGeneralRepairSound1"), true);
            zbarrier.upgradedGeneralRepairSound2.name =
                ctx.HashXHash(objCfg.GetCString("upgradedGeneralRepairSound2"), true);
            ...
            zbarrier.zombieReachThroughAnimState.name =
                ctx.HashXHash(objCfg.GetCString("zombieReachThroughAnimState"), true);
            ...
            if (!oboards.IsNull()) {
                ...
                for (auto& board : boards) {
                    ...
                    zboard.pauseAndRepeatRepSound.name =
                        ctx.HashXHash(cboard.GetCString("pauseAndRepeatRepSound"), true);
