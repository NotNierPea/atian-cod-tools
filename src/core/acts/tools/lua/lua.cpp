...
namespace tool::lua {
    ...
    class LuaDecompContext {
      public:
        ...
        int depth{};
        LuaDecompContext(CLIOption& opt, byte* file, size_t size)
            : opt(opt), file(file), size(size), header((LuaHeader*)file) {
        ...
        HKSInstId GetOpCodeId(uint64_t code) { return opcodes::GetOpCode(vm, code); }

        std::ostream& WriteAsmLocPadding(std::ostream& out, size_t floc = 0) {
            return utils::Padding(out, depth)
                   << "." << std::setfill('0') << std::setw(7) << std::hex << (floc ? floc : loc) << " ";
        }
    ...
    class HKSConstVal {
      public:
        ...
        virtual ~HKSConstVal() {}
        virtual void Dump(std::ostream& out) { out << "<INVALID>"; }
    };
    class HKSConstValNil : public HKSConstVal {
      public:
        HKSConstValNil() : HKSConstVal(HKSC_NIL) {}
        void Dump(std::ostream& out) override { out << "nil"; }
    };
    ...
    class HKSConstValHash : public HKSConstVal {
      public:
        ...
        void Dump(std::ostream& out) override { out << "@\"" << hashutils::ExtractTmp("hash", val) << "\""; }
    };
    ...
    class HKSConstValBool : public HKSConstVal {
      public:
        ...
        void Dump(std::ostream& out) override { out << val ? "true" : "false"; }
    };
    ...
    class HKSConstValNumber : public HKSConstVal {
      public:
        ...
        void Dump(std::ostream& out) override { out << val; }
    };
    ...
    class HKSConstValStr : public HKSConstVal {
      public:
        ...
        void Dump(std::ostream& out) override { utils::PrintFormattedString(out << "\"", val) << "\""; }
    };
    class HKSInstHandler {
      public:
        const char* name;
    ...
    struct LuaFuncHeader {
        ...
        bool Decompile(std::ostream& asmout, LuaDecompContext& ctx) {
            ...
            for (size_t i = 0; i < insts.size(); i++) {
                ...
                ctx.WriteAsmLocPadding(out, inst.loc)
                    << std::dec << i << " " << std::setfill('0') << std::setw(2) << (int)inst.code << " "
                    << std::setfill(' ') << std::setw(28) << std::left << GetOpCodeName(id) << std::right << " ";
        ...
        void Dump(std::ostream& out, LuaDecompContext& ctx) { DumpHeader(out, ctx); }
    };
    ...
    static bool HandleByteCode(byte* buffer, size_t size, const std::filesystem::path& origin, CLIOption& opt) {
        ...
        if (opt.m_header) {
            out << "-- file: " << origin.string() << "\n"
                ...
                << std::hex << "-- Version ..... 0x" << (int)header->version << " / 0x" << (int)header->compVersion
                << " 0x" << ctx.vm << "\n"
                << std::hex << "-- Endian ...... 0x" << (int)header->endian << "\n"
                << std::hex << "-- Size ........"
                << " sizeInt:0x" << (int)header->sizeInt << " sizeSize:0x" << (int)header->sizeSize << " sizeInst:0x"
                << (int)header->sizeInst << " sizeNumber:0x" << (int)header->sizeNumber << "\n"
