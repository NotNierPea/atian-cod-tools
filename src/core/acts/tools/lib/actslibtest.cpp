...
namespace {
    ...
    void MergeNumbers() {
        ...
        actslib::data::iterator::AllocatedMergeAIterator<size_t, Arr> merger{
            vec,
            [](Arr& e) {
                return std::make_shared<actslib::data::iterator::HandleAIterator<size_t, size_t*>>(e.start, e.end);
            }
        };
    ...
    int actslibtest(Process& proc, int argc, const char* argv[]) {
        ...
        if (!_strcmpi(type, "prof")) {
            ...
            std::unique_ptr<actslib::rdf::RDFParser> parserPtr =
                actslib::rdf::CreateParser(format, is, "http://atesab.fr/#");
    ...
    int proftocsv(int argc, const char* argv[]) {
        ...
        for (const std::filesystem::path& path : profs) {
            ...
            for (actslib::profiler::ProfilerSection::const_iterator itg = global.cbegin(); itg != global.cend();
                 itg++) {
                const actslib::profiler::ProfilerSection& main{ *itg };
                if (colms.empty()) {
                    os << "name";
                    for (actslib::profiler::ProfilerSection::const_iterator it = main.cbegin(); it != main.cend();
                         it++) {
