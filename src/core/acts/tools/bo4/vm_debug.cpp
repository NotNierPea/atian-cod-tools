...
namespace {
        ...
        case TYPE_STRUCT: {
            ...
            if (!proc.ReadMemory(&info, glob.scriptVariablesObjectInfo1 + sizeof(ScrVarObjectInfo1) * valId,
                ...
                if ((ptrvar.type == TYPE_ARRAY && deep <= opt.m_deep_array) ||
                    (ptrvar.type != TYPE_ARRAY && deep <= opt.m_deep_struct)) {
    ...
    int vmdebug(Process& proc, int argc, const char* argv[]) {
        ...
        if (vmError) {
            ...
            std::cout << "VM error: " << std::dec << vmErrorCode << " (0x" << std::hex << vmErrorCode << "): ";
        ...
        for (size_t inst = 0; inst < scriptinstance::SI_COUNT; inst++) {
            ...
            std::cout << std::dec << "--- " << scriptinstance::Name(inst) << " ---\n";
            ...
            if (opt.m_gvars) {
                ...
                for (const auto& gv : pub.globalVars) {
                    ...
                    out << "- " << hashutils::ExtractTmp("var", gv.name) << " id: " << std::dec << gv.id
                        << ", persist: " << (gv.persist ? "true" : "false");
            ...
            for (size_t i = vm.function_count; i > 0; i--) {
                ...
                if (!FindByteCodeLocationInfo((int)inst, proc, info, stack.bytecodeLocation, true)) {
                    ...
                    std::cout << std::hex << hashutils::ExtractTmpScript(info.obj.name) << std::flush << " ("
    ...
    int vmdebuglink(Process& proc, int argc, const char* argv[]) {
        ...
        for (size_t inst = 0; inst < scriptinstance::SI_COUNT; inst++) {
            ...
            uint32_t bufferCount =
                proc.ReadMemory<uint32_t>(proc[offset::gObjFileInfoCount] + sizeof(bufferCount) * inst);
        ...
        for (size_t inst = 0; inst < scriptinstance::SI_COUNT; inst++) {
            for (auto& [name, scriptVal] : scripts[inst]) {
                ...
                tool::gsc::T8GSCExport* exports =
                    (tool::gsc::T8GSCExport*)(prime_obj->magic + prime_obj->export_table_offset);
                ...
                for (size_t i = 0; i < prime_obj->include_count; i++) {
                    ...
                    tool::gsc::T8GSCExport* exports_using =
                        (tool::gsc::T8GSCExport*)(spt->magic + spt->export_table_offset);
                ...
                tool::gsc::T8GSCImport* imports =
                    (tool::gsc::T8GSCImport*)(prime_obj->magic + prime_obj->imports_offset);
                ...
                for (size_t i = 0; i < prime_obj->imports_count; i++) {
                    ...
                    bool findDevCall{};
                    if ((imp->flags & tool::gsc::T8GSCImportFlags::GET_CALL) != 0 ||
                        imp->import_namespace == 0xC1243180 || imp->import_namespace == 0x222276A9) {
                        if (import_type == tool::gsc::T8GSCImportFlags::FUNC_METHOD ||
                            import_type == tool::gsc::T8GSCImportFlags::FUNCTION) {
                            // CScr_GetFunction
                        ...
                        if (import_type == tool::gsc::T8GSCImportFlags::FUNC_METHOD ||
                            import_type == tool::gsc::T8GSCImportFlags::METHOD) {
                            // &meth or <x> meth()
