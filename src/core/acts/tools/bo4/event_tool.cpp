...
namespace {
    ...
    void bo4_event_tool() {
        ...
        static const char* op1s[]{
            "season_2_stream", "season_3_stream", "season_4_stream", "season_5_stream",
            "season_6_stream", "season_7_stream", "season_8_stream",
        };
        static const char* op2s[]{
            "", "season_2_bonus_stream", "summer_break_bundle", "fourth_of_july_event_stream", "labor_day_event_stream",
        };
        ...
        if (ImGui::Button("Inject")) {
            ...
            try {
                ...
                for (size_t i = 0; i < pe->itemAllocCount; i++) {
                    if (entries[i].name.name != entry.name.name)
                        continue; // not ours
