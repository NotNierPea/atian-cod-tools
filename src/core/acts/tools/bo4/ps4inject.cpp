...
namespace {
    ...
    int ps4inject(Process& _, int argc, const char* argv[]) {
        ...
        std::cout << "Ip: " << ipd
                  ...
                  << "\n"
                     "target: "
                  << target << " (script_" << std::hex << targetHash
                  << ")\n"
                     "replaced: "
                  << replaced << " (script_" << std::hex << replacedHash << ")\n";
        ...
        try {
            ...
            auto* targetEntry =
                std::find_if(scriptEntry, end, [targetHash](const auto& ent) { return ent.name == targetHash; });
            ...
            auto targetScriptIncludes =
                ps4.ReadMemory(pid, includesOffset, sizeof(uint64_t) * scriptHeader->include_count);
    ...
    int ps4t8cee(Process& _, int argc, const char* argv[]) {
        ...
        try {
            ...
            auto* targetGscEntry =
                std::find_if(scriptEntry, end, [](const auto& ent) { return ent.name == zm_utilityGsc; });
            auto* targetCscEntry =
                std::find_if(scriptEntry, end, [](const auto& ent) { return ent.name == zm_utilityCsc; });
            ...
            // Align 0x00
            // Return 0x0757
            std::vector<byte> payload{ 0x0d, 0x00, 0x89, 0x08, 0x01, 0x00, 0x57, 0x07 };
    ...
    int ps4injectcw100(Process& _, int argc, const char* argv[]) {
        ...
        try {
            ...
            auto targetScriptIncludes =
                ps4.ReadMemory(pid, includesOffset, sizeof(uint64_t) * scriptHeader->includes_count);
            auto includes = reinterpret_cast<uint64_t*>(targetScriptIncludes.data());
...
ADD_TOOL(ps4_t8cee, "bo4", " [ip:port]", "Enable EEs in custom mutation/offline/casual (BO4 / PS4 126)", nullptr,
         ps4t8cee);
ADD_TOOL(ps4_inject, "bo4", " [ip:port] [script] [target] [replaced]", "Inject script (BO4 / PS4 126)", nullptr,
         ps4inject);
ADD_TOOL(ps4_injectcw100, "cw", " [ip:port] [script] [target] [replaced]", "Inject script (CW / PS4 100)", nullptr,
         ps4injectcw100);