...
namespace {
    ...
    struct scr_weapon_field_t {
        ...
        uintptr_t
            getter; // typedef void (*ScriptCallbackWeapon)(scriptInstance_t, WeaponDef, const scr_weapon_field_t *);
    };
    ...
    int wfdump(Process& proc, int argc, const char* argv[]) {
        ...
        while (true) {
            ...
            of << "\n"
               ...
               << (field.isreadonly ? "true" : "false") << "," << std::hex << "0x" << field.ofs << ","
               << "0x" << field.size << ","
        ...
        for (size_t i = 0; i < 2; i++) {
            ...
            for (const auto& f : fields[i]) {
                ...
                if (!estimated) {
                    hof << "// can't read def type at " << std::hex << current
                        << ", type=" << (f.type < ACTS_ARRAYSIZE(fieldTypeNames) ? fieldTypeNames[f.type] : "invalid")
                        << "\n";
                    current += f.size;
                    continue;
                }
