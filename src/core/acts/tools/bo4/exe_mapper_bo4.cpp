...
namespace {
    ...
    void Com_Error_Stub(uint32_t code) { throw std::runtime_error(utils::va("Com_Error(0x%x)", code)); }

    const char* Sys_GetAbsZoneDir_Stub() { return utils::va("%s/zone", bo4.locationDir); }
    ...
    int bo4_exe_mapper(int argc, const char* argv[]) {
        ...
        // DB_LoadXFile(char const*, int, XZoneBuffer*, char const*, XAssetList*, XBlock*, void (*)(void), uchar*,
    ...
    int bo4_dump_rsa_key_loc(int argc, const char* argv[]) {
        ...
        if (!_strcmpi(argv[4], "mw3")) {
            DB_LoadRsaKey = mod->ScanSingle("E8 ?? ?? ?? ?? 4C 8D 44 24 60 BA 0E 01 00 00", "DB_GetRsaKey")
                                .GetRelative<int32_t, decltype(DB_LoadRsaKey)>(1);
            bufferSize = 270;
        } else {
            LOG_ERROR("Bad type");
            return tool::BAD_USAGE;
