...
namespace {
    ...
    int PTSearch(Process& proc, int argc, const char* argv[]) {
        ...
        if (argc <= 4) {
            for (const auto& mod : proc.modules()) {
                std::cout << std::hex << mod.path << " (" << mod.name << ") 0x" << std::hex << mod.start << " "
        ...
        std::cout << std::hex << mod.path << " (" << mod.name << ") 0x" << std::hex << mod.start << " " << std::dec
                  << mod.size << " Bytes\n";
        ...
        if (argc <= 5) {
            for (const auto& exp : mod.exports()) {
                std::cout << std::dec << exp.m_ordinal << ":" << exp.m_name << " " << mod.name << "+" << std::hex
                          << (exp.m_location - mod.start) << "\n";
            }
            return OK;
        }
    ...
    void TestFunc3() { std::cout << "Hello world\n"; }
    ...
    int PTCallMessage(Process& proc, int argc, const char* argv[]) {
        ...
        // memapi::Call<uintptr_t, const char*, const char*, DWORD>(proc, reinterpret_cast<uintptr_t>(&TestCallMB), 0,
        // argv[4], "test", MB_ICONINFORMATION);
