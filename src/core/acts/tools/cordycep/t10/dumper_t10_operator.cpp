...
namespace {
    ...
    class UnlinkerImpl : public Unlinker {
      public:
        ...
        virtual bool Unlink(const compatibility::scobalula::csi::XAsset64& asset, UnlinkerContext& ctx) {
            ...
            std::filesystem::path loc{ ctx.outDir / "tables" / "operator" /
    ...
    class UnlinkerSkinImpl : public Unlinker {
      public:
        ...
        virtual bool Unlink(const compatibility::scobalula::csi::XAsset64& asset, UnlinkerContext& ctx) {
            ...
            std::filesystem::path loc{ ctx.outDir / "tables" / "operator" / "skin" /
                                       std::format("{}.json", hashutils::ExtractTmp("file", data.name)) };
            if (opt.m_ignoreOld && std::filesystem::exists(loc))
                return true;
            std::filesystem::create_directories(loc.parent_path());
    ...
    class UnlinkerListImpl : public Unlinker {
      public:
        ...
        virtual bool Unlink(const compatibility::scobalula::csi::XAsset64& asset, UnlinkerContext& ctx) {
            ...
            std::filesystem::path loc{ ctx.outDir / "tables" / "operator" / "list" /
                                       std::format("{}.json", hashutils::ExtractTmp("file", data.name)) };
            if (opt.m_ignoreOld && std::filesystem::exists(loc))
                return true;
            std::filesystem::create_directories(loc.parent_path());
    ...
    utils::MapAdder<UnlinkerSkinImpl, bo6::T10RAssetType, Unlinker> impls{ GetUnlinkers(),
                                                                           bo6::T10R_ASSET_OPERATORSKIN };
    utils::MapAdder<UnlinkerListImpl, bo6::T10RAssetType, Unlinker> impll{ GetUnlinkers(),
                                                                           bo6::T10R_ASSET_OPERATORLIST };
} // namespace