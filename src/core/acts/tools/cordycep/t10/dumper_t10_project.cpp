...
namespace {
      ...
      public:
        ...
        virtual bool Unlink(const compatibility::scobalula::csi::XAsset64& asset, UnlinkerContext& ctx) {
            ...
            std::filesystem::path loc =
                ctx.outDir / "tables" / "project" / std::format("{}.json", hashutils::ExtractTmp("file", entry.name));
    ...
    class TableUnlinkerImpl : public Unlinker {
      public:
        ...
        virtual bool Unlink(const compatibility::scobalula::csi::XAsset64& asset, UnlinkerContext& ctx) {
            ...
            std::filesystem::path loc = ctx.outDir / "tables" / "project" / "table" /
            ...
            if (data.projectsCount) {
                ...
                for (size_t i = 0; i < data.projectsCount; i++) {
                    os << "\n"
                       << std::dec << i << ",#"
                       << hashutils::ExtractTmp("hash", proc.ReadMemory<uint64_t>(projects[i]));
                }
            }
    ...
    utils::MapAdder<TableUnlinkerImpl, bo6::T10RAssetType, Unlinker> implt{ GetUnlinkers(),
                                                                            bo6::T10R_ASSET_PROJECTTABLE };
} // namespace