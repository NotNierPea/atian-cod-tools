        ...
        case SBT_ARRAY_INDEXED: {
            ...
            if (arrayIndexed->count) {
                ...
                for (size_t i = 0; i < arrayIndexed->count; i++) {
                    if (i)
                        os << ",";
                    utils::Padding(os << "\n", depth + 1) << "\"#" << hashutils::ExtractTmp("hash", names1[i]) << ":#"
    ...
    bool sbv2::WriteBundleDef(
        ...
        case SBT_STRUCT: {
            ...
            if (obj->count) {
                ...
                for (size_t i = 0; i < obj->count; i++) {
                    ...
                    if (!WriteBundleDef(opt, json, proc, vals[i], GetLocalized))
                        return false;
                }
            }
        ...
        case SBT_ARRAY: {
            ...
            if (array->count) {
                ...
                for (size_t i = 0; i < array->count; i++) {
                    if (!WriteBundleDef(opt, json, proc, defs[i], GetLocalized))
                        return false;
                }
            }
    ...
    bool sbv2::WriteBundleData(
        ...
        if (data.count) {
            ...
            for (size_t i = 0; i < data.count; i++) {
                json.WriterFieldNameHash(names[i]);
                if (!WriteBundleDef(opt, json, proc, objects[i], GetLocalized))
                    return false;
            }
        }
        json.EndObject();
      ...
      public:
        ...
        virtual bool Unlink(const compatibility::scobalula::csi::XAsset64& asset, UnlinkerContext& ctx) {
            ...
            if (opt.m_usev1) {
                ...
                std::filesystem::path out = ctx.outDir / "dump" / "scriptbundle" /
                                            std::format("{}.json", hashutils::ExtractTmp("file", entry.name));
                if (opt.m_ignoreOld && std::filesystem::exists(out))
                    return true;
                ...
                if (name) {
                    name =
                        utils::MapString(utils::va("%s.json", name), [](char c) -> char { return c == ':' ? '/' : c; });
                } else {
