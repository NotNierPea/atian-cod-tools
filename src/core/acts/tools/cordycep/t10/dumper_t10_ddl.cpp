    ...
    class UnlinkerImpl : public Unlinker {
      public:
        ...
        virtual bool Unlink(const compatibility::scobalula::csi::XAsset64& asset, UnlinkerContext& ctx) {
            ...
            if (!dfilename)
                dfilename = utils::va("hashed/ddl/file_%llx.ddl", entry.name);
            ...
            while (true) {
                ...
                utils::Padding(os, 1) << "// enums ..... " << std::dec << def.enumsCount << " (0x" << std::hex
                                      << def.enumsCount << ")\n";
                utils::Padding(os, 1) << "// structs ... " << std::dec << def.structsCount << " (0x" << std::hex
                                      << def.structsCount << ")\n";
                utils::Padding(os, 1) << "// bit size .. " << std::dec << def.headerBitSize << " (0x" << std::hex
                                      << def.headerBitSize << ")\n";
                utils::Padding(os, 1) << "// byte size . " << std::dec << def.headerByteSize << " (0x" << std::hex
                                      << def.headerByteSize << ")\n";
                    ...
                    if (stct.memberCount && stct.members) {
                        ...
                        for (size_t i = 0; i < stct.memberCount; i++) {
                            ...
                            case DDL_STRING_TYPE:
                                os << "string(" << std::dec
                                   << ((member.bitSize / (member.isArray ? member.arraySize : 1)) / 8) << ")";
                                break;
                            ...
                            case DDL_PAD_TYPE:
                                ...
                                break;
                            default:
                                os << "<unk" << std::hex << (int)member.type << ">";
                                break;
                            }
                ...
                for (size_t i = 1; i < def.structsCount; i++) {
                    ...
                    utils::Padding(os, 1) << "// bitSize: 0x" << std::hex << stct.bitSize << ", members: " << std::dec
                                          << stct.memberCount << "\n";
                    utils::Padding(os, 1)
                        << "struct " << opt.AddString(proc.ReadStringTmp(reinterpret_cast<uintptr_t>(stct.name)))
                        << " {\n";
                    if (!DumpDDLStructMembers(stct, 2))
                        return false;
                    utils::Padding(os, 1) << "};\n\n";
                }
                ...
                for (size_t i = 0; i < def.enumsCount; i++) {
                    ...
                    utils::Padding(os, 1) << "// idx 0x" << std::hex << i << " members: 0x" << enm.count
                                          << " type: " << (int)enm.type << "\n";
                    utils::Padding(os, 1)
                        << "enum " << opt.AddString(proc.ReadStringTmp(reinterpret_cast<uintptr_t>(enm.name)))
                        << " {\n";
                ...
                utils::Padding(os, 1) << "// root: bitSize: 0x" << std::hex << root.bitSize << ", members: " << std::dec
                                      << root.memberCount << "\n\n";
                if (!DumpDDLStructMembers(structList[0], 1))
                    return false;
                os << "};\n\n";
