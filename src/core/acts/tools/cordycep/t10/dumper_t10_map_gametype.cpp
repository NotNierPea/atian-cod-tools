...
namespace {
    ...
    class UnlinkerImpl : public Unlinker {
      public:
        ...
        virtual bool Unlink(const compatibility::scobalula::csi::XAsset64& asset, UnlinkerContext& ctx) {
            ...
            json.WriteFieldNameString("unk50"); json.WriteValueString(std::format("({}, {}, {})", data.unk50[0],
            data.unk50[1], data.unk50[2])); json.WriteFieldNameString("unk5c"); json.WriteValueString(std::format("({},
            {}, {})", data.unk5c[0], data.unk5c[1], data.unk5c[2])); json.WriteFieldNameString("unk68");
            json.WriteValueNumber(data.unk68); json.WriteFieldNameString("unk6c"); json.WriteValueNumber(data.unk6c);
            ...
            */
            ...
            std::filesystem::path loc =
                ctx.outDir / "tables" / "map" / std::format("{}.json", hashutils::ExtractTmp("file", data.name));
    ...
    class TableUnlinkerImpl : public Unlinker {
      public:
        ...
        virtual bool Unlink(const compatibility::scobalula::csi::XAsset64& asset, UnlinkerContext& ctx) {
            ...
            std::filesystem::path loc = ctx.outDir / "tables" / "map" / "table" /
                                        std::format("{}.csv", hashutils::ExtractTmp("file", data.name));
            if (opt.m_ignoreOld && std::filesystem::exists(loc))
                return true;
            std::filesystem::create_directories(loc.parent_path());
            ...
            if (data.mapsCount) {
                ...
                for (size_t i = 0; i < data.mapsCount; i++) {
                    ...
                    os << "\n"
                       << std::dec << i << ",#" << hashutils::ExtractTmp("hash", mapInfo.name) << ","
                       << mapInfo.nameStr;
                }
            }
    ...
    class TableGTUnlinkerImpl : public Unlinker {
      public:
        ...
        virtual bool Unlink(const compatibility::scobalula::csi::XAsset64& asset, UnlinkerContext& ctx) {
            ...
            std::filesystem::path loc = ctx.outDir / "tables" / "gametype" / "table" /
                                        std::format("{}.csv", hashutils::ExtractTmp("file", data.name));
            if (opt.m_ignoreOld && std::filesystem::exists(loc))
                return true;
            std::filesystem::create_directories(loc.parent_path());
    ...
    utils::MapAdder<TableGTUnlinkerImpl, bo6::T10RAssetType, Unlinker> implgt{ GetUnlinkers(),
                                                                               bo6::T10R_ASSET_GAMETYPETABLE };
} // namespace