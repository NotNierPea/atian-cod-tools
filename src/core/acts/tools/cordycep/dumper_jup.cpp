...
namespace {
    ...
    namespace jupv1 {
        ...
        bool WriteBundleDef(
                ...
                if (arrayIndexed.count) {
                    ...
                    for (size_t i = 0; i < arrayIndexed.count; i++) {
                        ...
                        if (i)
                            os << ",";
                        utils::Padding(os << "\n", depth + 1)
                            << "\"#" << hashutils::ExtractTmp("hash", val.name) << "\": ";
                        if (!WriteBundleDef(opt, os, proc, entry, val.def, depth + 1))
                            return false;
                    }
                }
                ...
                if (arrayIndexed.count) {
                    ...
                    for (size_t i = 0; i < arrayIndexed.count; i++) {
                        ...
                        if (i)
                            os << ",";
                        utils::Padding(os << "\n", depth + 1)
                            << "\"#" << hashutils::ExtractTmp("hash", val.name1) << ":#"
                            << hashutils::ExtractTmp("hash", val.name2) << "\": ";
                        if (!WriteBundleDef(opt, os, proc, entry, *def, depth + 1))
                            return false;
                    }
                }
                ...
                if (array.count) {
                    ...
                    for (size_t i = 0; i < array.count; i++) {
                        if (i)
                            os << ",";
                        utils::Padding(os << "\n", depth + 1);
                        if (!WriteBundleDef(opt, os, proc, entry, defs[i], depth + 1))
                            return false;
                    }
                }
            ...
            case SBT_STRUCT_ANIMATION: {
                ...
                utils::Padding(os << "\n", depth + 1) << "\"#name\": \"" << name << "\",";
                utils::Padding(os << "\n", depth + 1)
                    << "\"#id\": \"anim#" << hashutils::ExtractTmp("hash", anim.anim) << "\"";
                utils::Padding(os << "\n", depth) << "}";
                break;
            }
    ...
    namespace jupv2 {
        ...
        bool WriteBundleDef(
            ...
            case SBT_STRUCT: {
                os << "{";
                std::unique_ptr<ScriptBundleObjectData> obj =
                    proc.ReadMemoryObjectEx<ScriptBundleObjectData>(def.value.obj);

                if (obj->count) {
                    std::unique_ptr<uint64_t[]> names = proc.ReadMemoryArrayEx<uint64_t>(obj->names, obj->count);
                    std::unique_ptr<ScriptBundleObjectDef[]> vals =
                        proc.ReadMemoryArrayEx<ScriptBundleObjectDef>(obj->defs, obj->count);
                    ...
                    for (size_t i = 0; i < obj->count; i++) {
                        ...
                        if (i)
                            os << ",";
                        utils::Padding(os << "\n", depth + 1)
                            << "\"#" << hashutils::ExtractTmp("hash", names[i]) << "\": ";
                        if (!WriteBundleDef(opt, os, proc, entry, vals[i], depth + 1))
                            return false;
                    }
                }
            ...
            case SBT_ARRAY_INDEXED: {
                os << "[";
                auto arrayIndexed =
                    proc.ReadMemoryObjectEx<ScriptBundleObjectDefValueArrayIndexed>(def.value.arrayIndexed);
                ...
                if (arrayIndexed->count) {
                    ...
                    for (size_t i = 0; i < arrayIndexed->count; i++) {
                        if (i)
                            os << ",";
                        utils::Padding(os << "\n", depth + 1)
                            << "\"#" << hashutils::ExtractTmp("hash", names1[i]) << ":#"
                            << hashutils::ExtractTmp("hash", names2[i]) << "\": ";
                        if (!WriteBundleDef(opt, os, proc, entry, vals[i], depth + 1))
                            return false;
                    }
                }
    ...
    int dpcordimpl(Process& proc, compatibility::scobalula::csi::CordycepProc& cordycep, int argc, const char* argv[]) {
        ...
        HandlePool(ASSET_LUAFILE, luaDir, [&proc, luaDir](const XAsset64& asset, size_t count) -> bool {
            ...
            if (!na)
                na = utils::va("hashed/lua_%llx.lua", entry.name);
        ...
        HandlePool(ASSET_STRINGTABLE, dumpDir, [&opt, &proc, dumpDir](const XAsset64& asset, size_t count) -> bool {
            ...
            if (!filename)
                filename = utils::va("hashed/stringtable/file_%llx.%s", entry.name, opt.m_cf_files ? ".cf" : "csv");
        ...
        HandlePool(ASSET_SCRIPTBUNDLE, dumpDir, [&opt, &proc, dumpDir](const XAsset64& asset, size_t count) -> bool {
            if (opt.m_usev1) {
                ...
                if (!filename)
                    filename = utils::va("file_%llx.%s", entry.name, opt.m_cf_files ? ".cf" : "json");
        ...
        HandlePool(ASSET_DDL, dumpDir, [&opt, &proc, dumpDir](const XAsset64& asset, size_t count) -> bool {
            ...
            if (!dfilename)
                dfilename = utils::va("hashed/ddl/file_%llx.%s", entry.name, opt.m_cf_files ? ".cf" : "ddl");
                ...
                while (toParse.size()) {
                    ...
                    if (stct.memberCount && stct.members) {
                        ...
                        for (size_t i = 0; i < stct.memberCount; i++) {
                            ...
                            utils::Padding(os, 1)
                                << "// offset: 0x" << std::hex << member.offset << ", bitSize: 0x" << member.bitSize;
                            ...
                            case DDL_PAD_TYPE:
                                ...
                                break;
                            default:
                                os << "<invalid:" << std::dec << (int)member.type << ">";
                                break;
                            }
                ...
                for (int id : toParseEnum) {
                    ...
                    if (enm.count && enm.members) {
                        ...
                        for (size_t i = 0; i < enm.count; i++) {
                            utils::Padding(os, 1) << "\"" << opt.AddString(proc.ReadStringTmp(members[i])) << "\" = 0x"
                                                  << std::hex << i << ",\n";
                        }
