...
namespace tool::cordycep::dump::t10 {
    ...
    namespace {
        ...
        int dpcordimpl(Process& proc, compatibility::scobalula::csi::CordycepProc& cordycep, int argc,
            ...
            for (auto& [type, unlinker] : GetUnlinkers()) {
                HandlePool(type, outDir, [&uctx, &unlinker](const XAsset64& asset, size_t count) {
                    return unlinker->Unlink(asset, uctx);
                });
            }
            ...
            if (opt.m_dump_types[T10R_ASSET_LOCALIZE] || opt.m_dump_all_available) {
                ...
                os << "{";
                int du =
                    ForEachEntry(proc, pools[T10R_ASSET_LOCALIZE], [&proc, &os](const XAsset64& asset, size_t count) {
                        LocalizeEntry entry{};
                        if (!proc.ReadMemory(&entry, asset.Header, sizeof(entry))) {
                            LOG_ERROR("Can't read LocalizeEntry {:x}", asset.Header);
                            return false;
    ...
    // compatibility::scobalula::csi::CG_BO6, dpcordimpl, T10R_ASSET_COUNT };
} // namespace tool::cordycep::dump::t10