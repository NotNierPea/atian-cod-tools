    ...
    class UnlinkerImpl : public Unlinker {
      public:
        ...
        virtual bool Unlink(const compatibility::scobalula::csi::XAsset64& asset, UnlinkerContext& ctx) {
            ...
            if (!dfilename)
                dfilename = utils::va("file_%llx.ddl", entry.name);
            ...
            auto WriteHeader = [&os, &def]() {
                utils::Padding(os, 1) << "// enums ..... " << std::dec << def.numEnums << " (0x" << std::hex
                                      << def.numEnums << ")\n";
                utils::Padding(os, 1) << "// structs ... " << std::dec << def.numStructs << " (0x" << std::hex
                                      << def.numStructs << ")\n";
                utils::Padding(os, 1) << "// header bit size .. " << std::dec << def.headerBitSize << " (0x" << std::hex
                                      << def.headerBitSize << ")\n";
                utils::Padding(os, 1) << "// header byte size . " << std::dec << def.headerByteSize << " (0x"
                                      << std::hex << def.headerByteSize << ")\n";
            };
