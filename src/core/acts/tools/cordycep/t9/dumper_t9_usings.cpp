      ...
      public:
        ...
        virtual bool Unlink(const compatibility::scobalula::csi::XAsset64& asset, UnlinkerContext& ctx) {
            ...
            if (name) {
                ...
                loc = ctx.outDir / "tables" / "usings" / type / "hashed" /
                      std::format("file_{:x}.{}.json", entry.name, ext);
            }
            ...
            if (entry.serverName) {
                json.WriteFieldNameString("serverName");
                json.WriteValueHash(entry.serverName);
            }
            if (entry.clientName) {
                json.WriteFieldNameString("clientName");
                json.WriteValueHash(entry.clientName);
            }
            json.EndObject();
