...
namespace {
    ...
    struct GscObj23 {
        ...
        uint64_t GetMagic() { return *reinterpret_cast<uint64_t*>(magic); }
    };
    ...
    struct ExportCTX {
        ...
        void DumpHeader(std::ostream& asmout) {
            asmout << "// Namespace " << ctx.Extract("namespace", exp.name_space) << std::flush << "/"
                   << ctx.Extract("namespace", exp.file_name_space) << "\n"
        ...
        uint32_t RLoc() { return (uint32_t)(bytecode - file.magic); }
            ...
            while (true) {
                ...
                case 0x83: {
                    ...
                    WriteOperation(asmout, opcode, "unk")
                        << std::hex << (delta < 0 ? "-" : "") << (delta < 0 ? -delta : delta) << "\n";
                    break;
                }
                case 0x16:
                case 0x61:
    ...
    int handlefile(GscObj23* file, size_t size, const std::filesystem::path& path, Ctx23& ctx) {
        ...
        if (ctx.opt.m_header) {
            asmout << "// Name:     " << ctx.ExtractScript(file->name) << "(" << path << ")\n"
        ...
        if (ctx.opt.m_imports && file->imports_count) {
            ...
            for (size_t i = 0; i < file->imports_count; i++) {
                ...
                asmout << std::hex << ctx.Extract("namespace", imp->name_space) << "::" << std::flush
                       << ctx.Extract("function", imp->name) << " flags:" << (int)imp->flags << " params: " << std::dec
                       << (int)imp->param_count << "\n";
        ...
        if (ctx.opt.m_addresses) {
            ...
            if (file->unk30_count) {
                ...
                for (size_t i = 0; i < file->unk30_count; i++) {
                    ...
                    // 4 int address -> char*
                    // 8 int[]
                    asmout << reinterpret_cast<const char*>(file->magic + section30->address1) << "%"
                           << reinterpret_cast<const char*>(file->magic + section30->address2) << "\n"
                           << "add:";
    ...
    int gscinfo(Process& _, int argc, const char* argv[]) {
        ...
        for (const auto& path : files) {
            ...
            if (!(name.ends_with(".gscbin") || name.ends_with(".gscc") || name.ends_with(".cscbin") ||
                  name.ends_with(".cscc"))) {
        ...
        if (ctx.opt.m_dump_str) {
            ...
            if (ofs) {
                ...
                for (const auto& si : ctx.opt.m_dumpStrings) {
                    ofs << hashutils::ExtractTmpScript(si.script) << "\t" << (int)si.type << "\t" << (int)si.ref << "\t"
                        << si.str << "\n";
                }
            }
        }
