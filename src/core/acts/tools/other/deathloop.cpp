...
namespace {
    ...
    enum IdxType { IDT_UNK = 0, IDT_BASE, IDT_MASTER, IDT_MASTER_RES, IDT_RSC_DEPS, IDT_COUNT };
    const char* IdxTypeNames[]{ "unknown", "base", "master", "master_res", "rsc_deps" };
        ...
        case IDT_MASTER_RES: {
            ...
            for (size_t i = 0; i < numEntries; i++) {
                ...
                if (u9)
                    LOG_INFO("u9 0x{:x}", id);
                if (u10)
                    LOG_INFO("u10 0x{:x}", id);

                os << std::hex << "0x" << id << ",off:0x" << offset << ",ds:0x" << decompressedSize << ",cs:0x"
                   << compressedSize << ",fileId:0x" << (int)fileId << ",u7:" << (u7 ? "true" : "false") << ",u8:0x"
                   << (int)u8 << ",u10:0x" << (int)u10;

                os << "," << kty << "," << filename << "," << path << "\n";
        ...
        std::map<size_t, std::vector<ResourceFile>> locFiles{};
        {
            ...
            for (auto& [fileId, resFiles] : locFiles) {
                ...
                for (ResourceFile& rf : resFiles) {
                    ...
                    bool oodleCompressed{ rf.compressedSize > 12 && !std::memcmp("OOD", data, 3) &&
                                          (data[4] & 0xF) != 3 };
