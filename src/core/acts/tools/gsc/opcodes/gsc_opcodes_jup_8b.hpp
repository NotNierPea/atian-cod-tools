...
namespace tool::gsc::opcode {
    class OpCodeJup8BRegistry {
      public:
        static void OpCode() {
            VmInfo* v8b = RegisterVM(
                VMI_JUP_8B, "Call of Duty: Modern Warfare III (8B)", "jup", "mwiii",
                VmFlags::VMF_CRC_DUMP | VmFlags::VMF_FOREACH_IW | VmFlags::VMF_HASH64 | VmFlags::VMF_NO_PARAM_FLAGS |
                    VmFlags::VMF_FULL_FILE_NAMESPACE | VmFlags::VMF_HASH_IW | VmFlags::VMF_HASH_PATH_IW |
                    VmFlags::VMF_CALL_NO_PARAMS | VmFlags::VMF_IW_CALLS | VmFlags::VMF_IW_LIKE | VmFlags::VMF_ISTRING);
            ...
            v8b->RegisterVMOperatorFunction("waittill", "<caller> waittill(event, var*)", OPCODE_IW_SingleWaitTill,
                                            VPFD_SELF_PARAM | VPFD_UNPACK, 1);
            v8b->RegisterVMOperatorFunction("isdefined", "isdefined(object) -> bool", OPCODE_IsDefined,
                                            VPFD_RETURN_VALUE, 1, 1);
            v8b->RegisterVMOperatorFunction("notify", "<caller> notify(event, param*)", OPCODE_IW_Notify,
                                            VPFD_SELF_PARAM | VPFD_USE_PRE_SCRIPT_CALL, 1);
            v8b->RegisterVMOperatorFunction("endon", "<caller> endon(event)", OPCODE_IW_SingleEndon, VPFD_SELF_PARAM, 1,
                                            1);
            ...
            v8b->RegisterVMOperatorFunction("getthread", "getthread() -> thread", OPCODE_IW_GetThread,
                                            VPFD_RETURN_VALUE, 0, 0);
            v8b->RegisterVMOperatorFunction("istrue", "istrue(object) -> bool", OPCODE_IW_IsTrue, VPFD_RETURN_VALUE, 1,
                                            1);
            v8b->RegisterVMOperatorFunction("waittillframeend", "waittillframeend()", OPCODE_WaitTillFrameEnd,
                                            VPFD_NONE, 0, 0);
            v8b->RegisterVMHashOPCode('#', OPCODE_GetHash, 8, [](const char* str) { return hash::Hash64(str); });
            v8b->RegisterVMHashOPCode('@', OPCODE_IW_GetDVarHash, 8,
                                      [](const char* str) { return hash::HashIWDVar(str); });
            v8b->RegisterVMHashOPCode('%', OPCODE_IW_GetResourceHash, 8,
            ...
            v8b->RegisterDatatype("builtinfunction", "builtinmethod", "function", "string", "istring", "struct", "int",
                                  "float", "vector");
