    ...
    class OpCodeT601Registry {
      public:
        static void OpCode() {
            ...
            // "vectorscale(vector, factor) -> vector", OPCODE_VectorScale, VPFD_RETURN_VALUE, 2, 2);
            // vt->RegisterVMOperatorFunction("waittill", "<caller> waittill(event, var*)", OPCODE_IW_SingleWaitTill,
            // VPFD_SELF_PARAM | VPFD_UNPACK, 1);
            //  vt->RegisterVMOperatorFunction("waittillmatch", "<caller> waittillmatch(event, match) -> struct",
            ...
            // vt->RegisterVMOperatorFunction("waittillframeend", "waittillframeend()", OPCODE_WaitTillFrameEnd,
            // VPFD_NONE, 0, 0); vt->RegisterVMHashOPCode('#', OPCODE_GetHash32, 4, [](const char* str) { return
            // hash::HashT7(str); }); vt->RegisterVMHashOPCode('s', OPCODE_GetHash32, 4, [](const char* str) { return
            // hash::HashT7(str); }); vt->RegisterDevCall("assert", "assertmsg", "errormsg", "throw", "println");
