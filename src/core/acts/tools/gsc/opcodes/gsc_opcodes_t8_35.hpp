...
namespace tool::gsc::opcode {
    class OpCodeT835Registry {
      public:
        static void OpCode() {
            VmInfo* vt835 = RegisterVM(VMI_T8_35, "Call of Duty: Black ops 4 (35)", "t8", "bo4_35",
                                       VmFlags::VMF_OPCODE_U16 | VmFlags::VMF_ALIGN | VmFlags::VMF_INV_ADD_TO_OBJECT |
                                           VmFlags::VMF_CLIENT_VM);
            ...
            vt835->RegisterVMOperatorFunction("isdefined", "isdefined(object) -> bool", OPCODE_IsDefined,
                                              VPFD_RETURN_VALUE, 1, 1);
            vt835->RegisterVMOperatorFunction("notify", "<caller> notify(event, param*)", OPCODE_Notify,
                                              VPFD_SELF_PARAM | VPFD_USE_PRE_SCRIPT_CALL, 1);
            vt835->RegisterVMOperatorFunction("endon", "<caller> endon(event+)", OPCODE_EndOn,
                                              VPFD_SELF_PARAM | VPFD_USE_COUNT, 1);
            vt835->RegisterVMOperatorFunction("endoncallback", "<caller> endoncallback(func, event+)",
                                              OPCODE_EndOnCallback, VPFD_SELF_PARAM | VPFD_USE_COUNT, 2);
            vt835->RegisterVMOperatorFunction("vectorscale", "vectorscale(vector, factor) -> vector",
                                              OPCODE_VectorScale, VPFD_RETURN_VALUE, 2, 2);
            vt835->RegisterVMOperatorFunction("waittill", "<caller> waittill(event) -> struct", OPCODE_WaitTill,
                                              VPFD_SELF_PARAM | VPFD_USE_COUNT | VPFD_RETURN_VALUE, 1);
            vt835->RegisterVMOperatorFunction("waittillmatch", "<caller> waittillmatch(event, match) -> struct",
                                              OPCODE_WaitTillMatch,
                                              VPFD_SELF_PARAM | VPFD_USE_COUNT | VPFD_RETURN_VALUE, 2);
            vt835->RegisterVMOperatorFunction(
                "waittillmatchtimeout", "<caller> waittillmatchtimeout(event, match, timeout) -> struct",
                OPCODE_WaitTillMatchTimeout, VPFD_SELF_PARAM | VPFD_USE_COUNT | VPFD_RETURN_VALUE, 3);
            vt835->RegisterVMOperatorFunction("waittilltimeout", "<caller> waittilltimeout(event, timeout) -> struct",
            ...
            vt835->RegisterVMOperatorFunction("waittillframeend", "waittillframeend()", OPCODE_WaitTillFrameEnd,
                                              VPFD_NONE, 0, 0);
            ...
            vt835->RegisterDatatype("functionptr", "scriptfunctionptr", "codefunctionptr", "string", "array", "weapon",
                                    "int", "float", "vec", "class", "struct", "hash");
