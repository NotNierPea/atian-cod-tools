...
namespace tool::gsc::gdb {
    namespace {
        int gscgdbinfo(Process& proc, int argc, const char* argv[]) {
            ...
            for (auto& path : paths) {
                ...
                auto* lineInfo =
                    reinterpret_cast<GSC_LINEINFO*>(utils::Aligned<uint64_t>(obj->magic + obj->lineinfo_offset));
    ...
    int gscgdbc(Process& proc, int argc, const char* argv[]) {
        ...
        for (const std::filesystem::path& input : inputs) {
            ...
            std::filesystem::path outFir{ outDir /
                                          utils::va("%s%sgdb", gdb.nameHashed ? ".gsc" : "", gdb.nameStr.c_str()) };
