...
namespace tool::gsc::iw {
    ...
    static const char* GetDataFromCache(StoreCache& cache, uint64_t vm, const char* name, uint32_t str) {
        ...
        for (size_t i = 0; i < cache.count; i++) {
            ...
            if (str >= count)
                continue; // string too high
            uint32_t loc{ reader.ReadPtr<uint32_t>(count)[str] };
            if (!loc)
                continue; // not defined in this ext
...
namespace {
    int acef_gscopaque(int argc, const char* argv[]) {
        ...
        utils::compress::CompressionAlgorithm alg{ tool::NotEnoughParam(argc, 5)
                                                       ? compatibility::acts_acef::DEFAULT_COMPRESS
                                                       : utils::compress::GetConfigName(argv[6]) };
    ...
    int gscbin_recompress(int argc, const char* argv[]) {
        ...
        compatibility::xensik::gscbin::GscBinHeader* header{
            (compatibility::xensik::gscbin::GscBinHeader*)buff.data()
        };
        ...
        constexpr utils::compress::CompressionAlgorithm alg =
