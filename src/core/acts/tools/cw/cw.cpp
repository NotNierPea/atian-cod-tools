...
namespace {
    ...
    int ps4reader100(Process& _, int argc, const char* argv[]) {
        ...
        std::cout << std::hex << "spt ... " << pool->pool << "\n"
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            std::cout << std::dec << i << ": " << hashutils::ExtractTmpScript(e.name) << " -> " << std::hex << e.buffer
                      << " " << e.len << "\n";
    ...
    int cwdbgcn(int argc, const char* argv[]) {
        ...
        for (size_t i = 0; i < 40; i++) {
            ...
            os << "\n"
               << (bg->name ? mod->Rebase<const char>(bg->name) : "<invalid>") << "," << cw::PoolName(bg->assetType)
               << "," << hook::library::CodePointer{ bg->registerFunc } << ","
               << hashutils::ExtractTmp("hash", bg->hash) << "," << std::hex << "0x" << bg->allocItems << ","
               << std::hex << "0x" << bg->defaultEntryIndex << "," << std::hex << "0x" << bg->startIndex << ","
               << std::hex << "0x" << bg->numEntries << "," << std::hex << "0x" << bg->checksum;
        }
    ...
    int cwassoff(int argc, const char* argv[]) {
        ...
        DB_GetXAssetName =
            mod->ScanSingle("0F B6 C1 48 8D 0C 80 48 8D 05 ? ? ? ? 48 8B").GetPtr<decltype(DB_GetXAssetName)>();
