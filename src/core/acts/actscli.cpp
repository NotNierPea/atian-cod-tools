...
namespace {
    ...
    enum ActsPackMinVersion : byte { APMV_Hashes = 0x10, APMV_VMs = 0x10, APMV_FLAGS = 0x11, APMV_VM64s = 0x11 };
    ...
    struct ActsPack {
        ...
        constexpr bool HasFeature(ActsPackMinVersion feat) { return header.version >= feat; }

        constexpr bool HasFlag(uint64_t flags) { return HasFeature(APMV_FLAGS) && (this->flags & flags) == flags; }
    };
        ...
        if (pack->HasFeature(ActsPackMinVersion::APMV_VMs) && !pack->HasFlag(APF_IGNORE_OLD_VM)) {
            ...
            for (size_t i = 0; i < pack->vmCount; i++) {
                ...
                if (vm.platformsCount &&
                    vm.platformsOffset + sizeof(ActsPackVMPlatform) * vm.platformsCount > bufferSize) {
    ...
    int packfile(Process& proc, int argc, const char* argv[]) {
        ...
        auto doPackVm = [&vms](uint64_t vm, tool::gsc::opcode::Platform plt) {
            return vms.empty() || vms.contains(tool::gsc::NameLocated{ plt, vm }) ||
                   vms.contains(tool::gsc::NameLocated{ 0, vm });
        };
        auto doPackAnyVm = [&vmsAny](uint64_t vm) { return vmsAny.empty() || vmsAny.contains(vm); };
