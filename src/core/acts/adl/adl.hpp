...
namespace acts::compiler::adl {
    ...
    constexpr bool IsUnsignedType(ADLDataTypeId id) { return id & ADF_UNSIGNED; }

    constexpr bool IsIntType(ADLDataTypeId id) { return (id & ADF_PRIMITIVE_MASK) == ADF_PRIMITIVE_INT; }

    constexpr bool IsFloatType(ADLDataTypeId id) { return (id & ADF_PRIMITIVE_MASK) == ADF_PRIMITIVE_FLOAT; }

    constexpr bool IsNumberType(ADLDataTypeId id) { return IsIntType(id) || IsFloatType(id); }

    constexpr bool IsDefaultType(ADLDataTypeId id) { return (id & ADF_MASK) == ADF_DEFAULT; }
} // namespace acts::compiler::adl
...
namespace acts::compiler::adl {
    ...
    struct ADLData {
        ...
        ADLDataTypeId IdOfName(const std::string& name) const { return IdOfName(hash::Hash64(name.c_str())); }
        ...
        void WriteIntVal(byte* ptr, size_t offset, size_t size, int64_t v, bool isSigned) {
            ...
            case 8:
                ...
                break;
            default:
                throw std::runtime_error(utils::va("invalid write raw size %d", size));
            }
        }
            ...
            for (auto& [k, v] : member.GetObj()) {
                ...
                for (const ADLStructField& field : str.fields) {
                    ...
                    case ADF_ENUM: {
                        ...
                        if (eit == enums.end())
                            throw std::runtime_error("it exception enums");
