...
namespace acts::vm {
    ...
    struct ActScript {
        ...
        constexpr ScriptExport* Exports() const { return (ScriptExport*)&magic[exports_table]; }

        constexpr ScriptExport* ExportsEnd() const { return &Exports()[exports_count]; }

        constexpr uint32_t* Includes() const { return (uint32_t*)&magic[includes_table]; }

        constexpr uint32_t* IncludesEnd() const { return &Includes()[includes_count]; }

        constexpr ScriptImport* Imports() const { return (ScriptImport*)&magic[imports_table]; }

        constexpr ScriptImport* ImportsEnd() const { return &Imports()[imports_count]; }

        constexpr const char* GetName() const { return (const char*)&magic[name_offset]; }

        constexpr const char* GetString(uint32_t ref) const { return (const char*)&magic[ref]; }
    ...
    struct VmString {
        ...
        inline char* Str() { return (char*)(&this[1]); }
    };
