...
namespace acts::vm {
    const char* VmVarTypeNames[]{
        "UNDEFINED", "THREAD",          "POINTER",          "INTEGER", "FLOAT",  "HASH",   "PRECALL",
        "LOCATION",  "SCRIPT_FUNCTION", "BUILTIN_FUNCTION", "ARRAY",   "VECTOR", "STRUCT", "STRING",
    };
    ...
    void ActsVm::AssertThreadStarted() {
        if (!currentThread)
            throw std::runtime_error("No thread started");
    }
    ...
    VmString* ActsVm::GetStringEntry(VmRef ref) { return (VmString*)alloc.DataByRef(ref); }

    const char* ActsVm::GetString(VmRef ref) { return ActsVm::GetStringEntry(ref)->Str(); }
    ...
    uint16_t ActsVm::FindBuiltinFunction(uint64_t name) { return BAD_BUILTIN_ID; }

    uint16_t ActsVm::FindBuiltinMethod(uint64_t name) {
        for (size_t i = 0; i < callMethodsCount; i++) {
            if (callMethods[i].name == name)
                return (uint16_t)(i | FLAG_BUILTIN_METHOD);
        }
    ...
    void ActsVm::AddString(const char* str) { AddStringRef(CreateString(str)); }
    ...
    size_t ActsVm::GetScriptInfoId(const ScriptInfo* info) { return info - linkedScripts; }

    int ActsVm::LinkScript(const char* name, ScriptInfo** oInfo) { return LinkActsScript(GetActsScript(name), oInfo); }
