...
namespace cli::options {
    class CliOption {
      ...
      public:
        ...
        constexpr size_t Length() { return len; }

        constexpr core::logs::loglevel Level() { return lvl; }
        ...
        std::string StringLine(size_t namePadding) {
            ...
            ss << std::setw(delta) << std::setfill(' ') << std::left << " "
               << ": " << description;
    ...
    class CliOptions {
      ...
      public:
        ...
        bool NotEnoughParam(size_t count) const { return params.size() < count; }
        ...
        constexpr size_t ParamsCount() { return params.size(); }

        const char* operator[](size_t idx) { return params[idx]; }
    };
