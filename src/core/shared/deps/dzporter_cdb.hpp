...
namespace deps::dzporter::cdb {
    ...
    bool ReadCDBFiles(std::filesystem::path p, std::function<void(uint64_t hash, const char* str)> each,
                      std::function<void*(size_t len)> allocMemory = {}, std::mutex* loadMutex = nullptr);
    bool ReadCDBFile(std::filesystem::path p, std::function<void(uint64_t hash, const char* str)> each,
                     std::function<void*(size_t len)> allocMemory = {}, std::mutex* loadMutex = nullptr);
