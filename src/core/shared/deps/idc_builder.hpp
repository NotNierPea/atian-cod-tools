...
namespace deps::idc_builder {
    ...
    class IdcBuilder {
      ...
      public:
        ...
        inline void AddAddress(void* loc, const char* name, const char* type = nullptr) {
            AddAddress(GetRva(loc), name, type);
        }
        ...
        // add a name to a rva, might have a type, the rva will obtained from deps::idc_builder::GetRva
        inline void AddAddressEx(void* loc, const char* name, const char* flags, const char* type = nullptr) {
            AddAddressEx(GetRva(loc), name, flags, type);
        }
        // create an enum type
