...
namespace games::cod::asset_names {
    ...
    class AssetNames {
      ...
      public:
        ...
        void DumpFiles(const std::filesystem::path& dir, AssetDumpFileOptions* opts) {
            ...
            if (opts->dumpTypeHeader) {
...

                {
                    ...
                    for (size_t i = 0; i < typeMapsCount; i++) {
                        os << "\n    " << opts->assetPrefix << core::strings::GetCppIdentifier(typeNames[i]) << " = 0x"
                           << std::hex << i << ", // " << typeNames[i];
                    }
                    os << "\n};\n\n";
                }
                LOG_LVLF(opts->logLevel, "Dump assets data to {}", f.string());
            ...
            if (opts->dumpHashedHeader) {
...

                {
                    ...
                    for (size_t i = 0; i < typeMapsCount; i++) {
                        os << "\n    " << opts->assetHashedPrefix
                           << core::strings::GetCppIdentifier(sortedHashedName[i]) << " = 0x" << std::hex
                           << PoolId(sortedHashedName[i]) << ", // " << sortedHashedName[i];
                    }
                    os << "\n};\n";
                }
                LOG_LVLF(opts->logLevel, "Dump assets data to {}", f.string());
         ...
         */
        constexpr AssetId InvalidId() const { return (AssetId)typeMapsCount; }

        /*
         * @return the types count
         */
        constexpr size_t TypesCount() const { return typeMapsCount; }
         ...
         */
        bool ShouldHandle(AssetId type) { return handleList.Empty() || handleList[type]; }
         ...
         * @return name
         */
        const char* GetPoolName(AssetHashed hash) { return GetTypeName(GetExePoolId(hash)); }
         ...
         * @return hashed value
         */
        AssetHashed GetHashType(AssetId type) { return PoolId(GetTypeName(type)); }
         ...
         */
        AssetId GetExePoolId(const char* name) { return GetExePoolId(PoolId(name)); }
