...
namespace core::raw_file {
    ...
    enum DataType : byte { RFDT_END = 0, RFDT_STRING, RFDT_STRING_ENCRYPTED, RFDT_HASH, RFDT_HASH_ESCAPED, RFDT_COUNT };
    ...
    class RawFileWriter {
      ...
      public:
        ...
        inline RawFileWriter& WriteString(const std::string& str) { return WriteString(str.data(), str.size()); }
    ...
    class RawFileReader {
      ...
      public:
        ...
        uint32_t GetStringSize() { return *buffer.Ptr<uint32_t>(); }

        uint64_t GetHash() { return *buffer.Ptr<uint64_t>(); }

        char* GetStringRef() { return buffer.Ptr<char>(sizeof(uint32_t)); }
