...
namespace core::raw_file::json {
    enum JsonFormat { JFF_PRETTY, JFF_INLINE, JFF_REDUCED };
    ...
    class RawFileJsonWriter {
      ...
      public:
        ...
        void SetFormat(JsonFormat _format) { format = _format; }

        void WritePreData(bool forceNoInline = false) {
            if (!forceNoInline && inlined)
                return;
            inlined = false;
            if (!startStruct) {
                writer.WriteString(",");
        ...
        void WriteValueString(const std::string& str) { WriteValueString(str.c_str()); }
        ...
        void WriteValueLiteral(const std::string& val) { WriteValueLiteral(val.data()); }

        void WriteValueBool(bool val) { WriteValueLiteral(val ? "true" : "false"); }
        ...
        void WriteFieldValueString(int64_t name, const std::string& val) { WriteFieldValueString(name, val.c_str()); }
        ...
        void EndObject() {
            if (!depth)
                throw std::runtime_error("End array without creating one");
            depth--;
            if (!startStruct) {
                WriteNewLine();
            }
        ...
        std::vector<byte>& Build() { return writer.Build(); }
    };
