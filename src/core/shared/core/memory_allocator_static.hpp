...
namespace core::memory_allocator {
    ...
    class MemoryAllocatorStatic {
      ...
      public:
        ...
        void* DataByRef(DataRefType ref) { return &data[ref]; }
      ...
      public:
        DataRefType RefByData(void* ptr) { return (DataRefType)((byte*)ptr - (byte*)&data[0]); }
        DataRefType AllocRef(size_t len) {
            ...
            while (header->allocated || header->len < len) {
                if (!header->next)
                    throw std::runtime_error("Not enough data in the allocator");
            ...
            if (len + sizeof(MemoryAllocatorStaticHeader<DataRefType>) < header->len) {
                ...
                MemoryAllocatorStaticHeader<DataRefType>* next =
                    (MemoryAllocatorStaticHeader<DataRefType>*)((byte*)ptr + len);
        ...
        void* Alloc(size_t len) { return DataByRef(AllocRef(len)); }
        ...
        void Free(void* ptr) { FreeRef(RefByData(ptr)); }
