...
namespace core::hashes {
    ...
    void* AllocHashMemory(size_t len) { return core::shared_cfg::GetSharedConfig().hashes.alloc.Alloc<byte>(len); }

    const char* CloneHashStr(const char* str) { return core::shared_cfg::GetSharedConfig().hashes.alloc.CloneStr(str); }
