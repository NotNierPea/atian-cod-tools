...
namespace hook::library {
    ...
    class ScanResult {
      public:
        ...
        ScanResult(byte* location = nullptr, ScanEntry* entry = nullptr) : location(location), entry(entry) {
            if (entry) {
        ...
        inline Type Get(size_t offset = 0) const {
            if (entry)
                entry->location = location + offset;
    ...
    class Library {
      ...
      public:
        ...
        Library(const char* name, bool system, int32_t flags = 0)
            : hmod(system ? platform::LoadSystemShared(name, flags) : platform::LoadShared(name, flags)) {}

        Library(const std::string& name, bool system = false, int32_t flags = 0)
            : hmod(system ? platform::LoadSystemShared(name, flags) : platform::LoadShared(name, flags)) {}
        ...
        void SetModule(void* hmod) { this->hmod = hmod; }

        void ClearModule() { this->hmod = 0; }
        ...
        void SetModule(const std::filesystem::path& name, int32_t flags = 0) { SetModule(name.string(), false, flags); }
        ...
        constexpr bool operator==(const Library& other) const { return hmod == other.hmod; }

        constexpr operator bool() const { return hmod != 0; }

        constexpr void* operator*() const { return hmod; }

        inline void* operator[](size_t offset) const { return reinterpret_cast<byte*>(hmod) + offset; }

        inline void* operator[](const char* name) const { return platform::GetFunctionAddress(hmod, name); }

        inline const char* GetName() const { return GetLibraryName(hmod); }

        inline const char* GetPath() const { return GetLibraryPath(hmod); }

        inline const char* GetPDB() const { return LocatePDB(hmod); }
    ...
    class Detour {
      ...
      public:
        ...
        constexpr operator bool() const { return base; }

        /*
         * @return origin pointer
         */
        constexpr void* GetOrigin() const { return origin; }
    ...
    class CodePointer {
      ...
      public:
        ...
        constexpr void* operator*() const { return location; }
