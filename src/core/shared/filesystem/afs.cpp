...
namespace filesystem::afs {
    ...
    ActsFileSystemWriter::ActsFileSystemWriter() { utils::Allocate(data, sizeof(ActsFileSystemHeader)); }

    void ActsFileSystemWriter::WriteFile(const char* name, void* file, size_t len) {
        if (files.size() == INT32_MAX)
            throw std::runtime_error("too many files for this datastructure");
