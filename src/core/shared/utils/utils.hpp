...
namespace utils {
     ...
     * @return data location before write
     */
    inline void WriteValue(std::ostream& os, const void* ptr, size_t size) { os.write((const char*)ptr, size); }
    /*
     * Write a value into a stream
     * @param Type value type
     * @param os stream
    ...
    inline bool EqualIgnoreCase(const std::string& s1, const std::string& s2) {
        return EqualIgnoreCase(s1.c_str(), s2.c_str());
    }
     ...
     */
    inline char* CloneString(const char* str) { return va("%s", str); }
    ...
    class OutFileCE {
      ...
      public:
        ...
        OutFileCE(const char* f, bool failOpen = false, std::ios::openmode mode = std::ios::out) : os(f, mode) {
            if (failOpen && !os)
                throw std::runtime_error(std::format("Can't open {}", f));
        }
        OutFileCE(const std::filesystem::path& f, bool failOpen = false, std::ios::openmode mode = std::ios::out)
            : os(f, mode) {
            if (failOpen && !os)
                throw std::runtime_error(std::format("Can't open {}", f.string()));
        }
        ...
        operator std::ofstream&() { return os; }
    ...
    class InFileCE {
      ...
      public:
        ...
        InFileCE(const std::filesystem::path& f, bool failOpen = false, std::ios::openmode mode = std::ios::in)
            : is(f, mode) {
            if (failOpen && !is)
                throw std::runtime_error(std::format("Can't open {}", f.string()));
        }
        ...
        bool operator!() { return !is; }

        std::ifstream& operator*() { return is; }

        operator std::ifstream&() { return is; }
