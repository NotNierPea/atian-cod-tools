...
int64_t ProcessModule::GetRelativeOffset(uintptr_t ptr) const { return ptr - start; }

void ProcessMemory::Free() {
    if (*this)
        proc.FreeMemory(ptr, size);
}
...
Process::Process(HANDLE processHandle, DWORD pid)
    : m_invalid(ProcessModule(*this)), m_handle(processHandle), m_pid(pid) {
    if (!m_pid || !GetModuleAddress(m_pid, NULL, &m_modAddress, &m_modSize)) {
        m_modAddress = 0;
        m_modSize = 0;
    }
...
bool Process::operator!() const { return m_pid == 0; }

Process::operator bool() const { return m_pid != 0; }

uintptr_t Process::operator[](uint64_t offset) const { return m_modAddress + offset; }

int64_t Process::GetModuleRelativeOffset(uintptr_t ptr) const { return ptr - m_modAddress; }

uintptr_t Process::AllocateMemory(size_t size) const { return AllocateMemory(size, PAGE_READWRITE); }
...
bool Process::IsInsideModule(uintptr_t ptr) const { return ptr >= m_modAddress && ptr < m_modAddress + m_modSize; }
...
void ProcessModule::ComputeExports() {
    ...
    if (!m_parent.ReadMemory(&ntHeader, start + dosHeader.e_lfanew, sizeof(ntHeader)) ||
    ...
    for (size_t i = 0; i < exports.NumberOfNames; i++) {
        ...
        if (m_parent.ReadString(name, loc, 2000) < 0) {
            std::cerr << "Can't read name at loc 0x" << std::hex << loc << " " << std::dec << i << "/"
...
uintptr_t ProcessModule::operator[](size_t address) const { return start + address; }
...
std::ostream& operator<<(std::ostream& os, const ProcessModuleExport& obj) {
    ...
    if (obj.m_location >= obj.m_module.start) {
        return os << "[" << obj.m_name << "@" << obj.m_module.name << "+" << std::hex
                  << (obj.m_location - obj.m_module.start) << "]";
    }
    return os << "[" << obj.m_name << "@" << std::hex << obj.m_location << "]";
}

std::ostream& operator<<(std::ostream& os, const ProcessLocation& obj) { return obj.proc.WriteLocation(os, obj.loc); }

std::wostream& operator<<(std::wostream& os, const ProcessLocation& obj) { return obj.proc.WriteLocation(os, obj.loc); }
