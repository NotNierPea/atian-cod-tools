...
class ProcessMemory {
  ...
  public:
    ...
    operator bool() const { return ptr && size; }
...
class ProcessModuleExport {
  public:
    ...
    inline bool operator!() const { return m_location == 0; }
    // @return if the export is valid
    inline bool operatorbool() const { return m_location != 0; }
...
class ProcessModule {
  public:
    ...
    inline bool operator!() const { return handle == INVALID_HANDLE_VALUE || !handle; }
    // @return if the module is valid
    inline bool operatorbool() const { return handle != INVALID_HANDLE_VALUE && handle; }
     ...
     */
    inline const std::vector<ProcessModuleExport>& exports() const { return m_exports; }
...
class Process {
  public:
     ...
     * @return pointer or null
     */
    ProcessMemory AllocateMemoryRef(size_t size) const { return { *this, AllocateMemory(size), size }; }
    /*
     * Allocate memory in the process with protection
     * @param size Size to allocate
     * @param protection Protection
    ...
    std::pair<std::unique_ptr<Type[]>, bool> ReadMemoryArray(uintptr_t src, size_t count) const {
        std::pair<std::unique_ptr<Type[]>, bool> pair =
            std::make_pair<std::unique_ptr<Type[]>, bool>(std::make_unique<Type[]>(count), true);
    ...
    std::pair<std::unique_ptr<Type>, bool> ReadMemoryObject(uintptr_t src) const {
        std::pair<std::unique_ptr<Type>, bool> pair =
            std::make_pair<std::unique_ptr<Type>, bool>(std::make_unique<Type>(), true);
     ...
     */
    inline ProcessLocation GetLocation(uintptr_t location) const { return ProcessLocation{ *this, location }; }
     ...
     * @return the proccess modules
     */
    inline const std::vector<ProcessModule>& modules() const { return m_modules; }
    /*
     * @return the proccess modules
     */
    inline std::vector<ProcessModule>& modules() { return m_modules; }
    /*
     ...
     * @return main module
     */
    inline ProcessModule& GetMainModule() { return (*this)[nullptr]; }
    /*
     * @return handle
     */
     ...
     * @return pid
     */
    inline int32_t GetProcessId() const { return m_pid; }
     ...
     * @return scan result or 0 for no search
     */
    inline uintptr_t Scan(const char* pattern, int32_t start = 0) { return (*this)[nullptr].Scan(pattern, start); }

    /*
     * Set if the reader should ignore the protection
     * @param ignoreProtection value
     */
    void SetIgnoreProtection(bool ignoreProtection) { this->ignoreProtection = ignoreProtection; }
