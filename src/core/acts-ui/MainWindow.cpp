...
MainWindow::MainWindow(QWidget* parent) : QMainWindow(parent) {
    ...
    connect(ui.actionOpen, &QAction::triggered, this, [this]() {
        QString path = QFileDialog::getOpenFileName(this, tr("File"), QString(), tr("File (*.*)"));
    ...
    for (ui3::tools::AbstractUITool* tool : tools) {
        ...
        connect(tool->action, &QAction::triggered, this, [this, tool]() { tool->Activate(); });
    }
    ...
    QFuture<void> future = QtConcurrent::run([this] { ActsAPIHash_ReadDefaultHashFilesP(&this->progressHandler); });
...
void MainWindow::OpenFile(const QString& path) {
    ...
    for (ui3::tools::AbstractUITool* tool : ui3::tools::GetTools()) {
        ...
        if (tool->path && tool->name &&
            (path.compare(tool->name, Qt::CaseInsensitive) == 0 || path.compare(tool->id, Qt::CaseInsensitive) == 0)) {
            if (tool->needsInitialization) {
                RequiresInitialization([tool, path, this]() { tool->Activate(); });
            } else {
                tool->Activate();
            }
            return;
        ...
        for (const QString& ext : extensions) {
            if (!ext.isEmpty() && path.endsWith(ext, Qt::CaseInsensitive)) {
                if (tool->needsInitialization) {
                    RequiresInitialization([tool, path, this]() { tool->OpenFile(path); });
                } else {
                    tool->OpenFile(path);
                }
