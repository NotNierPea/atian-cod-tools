...
namespace systems::gsc::link {
    ...
    inline byte* Scr_FindDetourArray(GSC_ACTS_DETOUR* detours, GSC_ACTS_DETOUR* end, bo4::GSC_OBJ* obj, byte* origin,
                                     uint64_t script, uint32_t nsp, uint32_t func) {
    ...
    byte* Scr_FindDetour(bo4::scriptInstance_t inst, byte* origin, uint64_t script, uint32_t nsp, uint32_t func) {
        for (size_t i = 0; i < gObjGDBFileInfoCount[inst]; i++) {
            ...
            if (odbg->addons && odbg->addons->HasFeature(acts_addons::AAF_DETOUR)) {
                // addons
                byte* d{ Scr_FindDetourArray(odbg->addons->GetDetours(odbg->obj->magic),
                                             odbg->addons->GetDetoursEnd(odbg->obj->magic), odbg->obj, origin, script,
                                             nsp, func) };
                if (d) {
                    return d;
                }
            ...
            if (odbg->gdb && odbg->gdb->HasFeature(acts_debug::ADF_DETOUR)) {
                ...
                byte* d{ Scr_FindDetourArray(odbg->gdb->GetDetours(), odbg->gdb->GetDetoursEnd(), odbg->obj, origin,
                                             script, nsp, func) };
                if (d) {
                    return d;
                }
    ...
    namespace {
        ...
        inline void Scr_CheckDetour(objGDBFileInfo_t* dbgobj, bo4::GSC_OBJ* obj, GSC_ACTS_DETOUR* detour,
                                    GSC_ACTS_DETOUR* end) {
            for (; detour != end; detour++) {
                ...
                for (size_t i = 0; i < obj->imports_count; i++) {
                    ...
                    if (traceDetours) {
                        LOG_TRACE("[{}] Loading detour {}<{}>::{} with {}/{} patch(es)",
                                  ...
                                  core::hashes::ExtractTmp("function", detour->name), match, (int)imp->num_address);
                    }
                }
            }
        }
        ...
        void Scr_CheckDetour(objGDBFileInfo_t* dbgobj, bo4::GSC_OBJ* obj) {
            ...
            if (dbgobj->addons && dbgobj->addons->HasFeature(acts_addons::AAF_DETOUR)) {
                Scr_CheckDetour(dbgobj, obj, dbgobj->addons->GetDetours(dbgobj->obj->magic),
            ...
            for (; gObjCountLinked[inst] < count; gObjCountLinked[inst]++) {
                ...
                if (prime_obj->cseg_offset > sizeof(bo4::GSC_OBJ) + sizeof(uint64_t)) {
                    ...
                    if (addons->GetMagic() == acts_addons::MAGIC) {
                        ...
                        obj->obj = prime_obj;
                        obj->addons = addons;
                        LOG_TRACE("load acts addons data {} detours={} {}",
                                  core::hashes::ExtractTmp("script", filename->name), addons->detour_count, (void*)obj);
                    }
                }

                // load dbg data
                bo4::ScriptParseTreeDBG* dbg{
                    bo4::DB_FindXAssetHeader(bo4::XAssetType::ASSET_TYPE_SCRIPTPARSETREEDBG, &hash, false, -1).sptdbg
                };
                if (dbg && dbg->gdb && dbg->gdbLen >= sizeof(uint64_t) && dbg->gdb->GetMagic() == acts_debug::MAGIC) {
                    ...
                    if (!gdb->HasFeature(acts_debug::ADF_CHECKSUM)) {
                        LOG_WARNING("Found ADF file without checksum feature: {}",
                                    core::hashes::ExtractTmp("script", hash));
                    } else {
                        if (gdb->checksum != prime_obj->crc) {
                            LOG_ERROR("Found ADF file with bad checksum for file {}",
                                      core::hashes::ExtractTmp("script", hash));
                            continue;
                        }
                    }
                    ...
                    obj->gdb = gdb;
                    LOG_TRACE("load acts debug data {} {}", core::hashes::ExtractTmp("script", filename->name),
                              (void*)obj);
                }
        ...
        void Scr_GetGscExportInfo_Stub(bo4::scriptInstance_t inst, byte* codepos, const char** scriptname,
                                       int32_t* sloc, int32_t* crc, int32_t* vm) {
            ...
            if (gdb) {
                ...
                if (filename) {
                    // use debug data
                    sprintf_s(scriptnamebuffer[inst], "%s::%s:%d@%x", filename,
                              core::hashes::ExtractTmp("function", expit->name), line + 1, rloc - expit->address);
            ...
            if (scriptname) {
                if (expit) {
                    sprintf_s(scriptnamebuffer[inst], "%s::%s@%x", core::hashes::ExtractTmp("script", script_obj->name),
                              core::hashes::ExtractTmp("function", expit->name), rloc - expit->address);
                } else {
