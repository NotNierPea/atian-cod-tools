...
namespace bo4 {
    ...
    S_ANY Ref<XAssetHeader(bo4::XAssetType type, XHash* name, bool errorIfMissing, int waitTime)> DB_FindXAssetHeader{
        0x2EB75B0_a
    };
    ...
    S_ANY Ref<ScrVarIndex_t(scriptInstance_t inst, ScrVarIndex_t parentId, ScrVarNameIndex_t index)>
        ScrVar_NewVariableByIndex{ 0x2760440_a };
    S_ANY Ref<void(scriptInstance_t inst, ScrVarIndex_t id, ScrVarValue_t* value)> ScrVar_SetValue{ 0x27616B0_a };

    // gsc funcs
    S_ANY Ref<BuiltinFunction(uint32_t canonId, int* type, int* min_args, int* max_args)> CScr_GetFunction{
        0x1F13140_a
    };
    S_ANY Ref<BuiltinFunction(uint32_t canonId, int* type, int* min_args, int* max_args)> Scr_GetFunction{
        0x33AF840_a
    };
    ...
    S_ANY Ref<void(scriptInstance_t inst, EntRefUnion value, ClassNum classNum, LocalClientNum_t num)>
        ScrVm_AddEntityNum{ 0x276E8D0_a };
    S_ANY Ref<void(scriptInstance_t inst, uint32_t stringValue, int entnum, bool is_params)>
        ScrEvent_FireCallbackEventEnt{ 0x27454B0_a };
    S_ANY Ref<void(scriptInstance_t inst, ClassNum classnum, uint32_t canonId, int32_t offset)> ScrVar_AddClassFields{
        0x2759C50_a
    };
    ...
    S_ANY Ref<scr_entref_t*(scr_entref_t* ent, scriptInstance_t inst, unsigned int index)> ScrVm_GetEntityRef{
        0x2773170_a
    };
    ...
    S_ANY Ref<dvar_t*(XHash* name, const char* desc, dvarType_t type, int flags, DvarValue* value, DvarLimits* domain)>
        Dvar_Register{ 0x3CEFAF0_a };
    ...
    S_ANY Ref<void(XHash* cmdName, xcommand_t function, cmd_function_t* allocedCmd)> Cmd_AddCommandInternal{
        0x3CDEE80_a
    };
#define Cmd_AddCommand(name, func)                                                                                     \
    ...
    S_ANY Ref<float(const LocalClientNum_t localClientNum, const char* text, int maxChars, Font* font)> R_TextWidth{
        0x35B2530_a
    };
    S_ANY Ref<uint32_t(const LocalClientNum_t localClientNum, const char* text, int maxChars, Font* font, float scale)>
    ...
    S_ANY Ref<void(ScopedCriticalSection* sec, int32_t s, ScopedCriticalSectionType type)>
        ScopedCriticalSectionConstructor{ 0x289E3C0_a };
