...
namespace {
    ...
    byte* FindExport(scriptinstance::ScriptInstance inst, uint64_t target_script, uint32_t name_space, uint32_t name) {
        ...
        auto its = std::find_if(gObjFileInfo[inst], end, [target_script](const cw::ObjFileInfo& info) {
        ...
        auto ite = std::find_if(reinterpret_cast<cw::T8GSCExport*>(obj->magic + obj->exports_tables), exportEnd,
                                [name_space, name](const cw::T8GSCExport& exp) {
    ...
    void VM_OP_LazyLink_Handler(scriptinstance::ScriptInstance inst, void* varInfo, cw::FunctionStack* fs_0, void* ctx,
                                bool* terminate) {
        struct LazyLinkData {
            uint32_t nameSpace;
            uint32_t name;
    ...
    void PostInit(uint64_t id) {
        ...
        auto objFile = hook::library::QueryScanContainerSingle(
            "gObjFileInfo", "4C 8D 2D ? ? ? ? 48 8D 15 ? ? ? ? 43 39 4C B5 00"); // count / table
#ifndef CI_BUILD

        DB_FindXAssetHeader = hook::library::QueryScanContainerSinglePtr<decltype(DB_FindXAssetHeader)>(
            "DB_FindXAssetHeader", "48 89 74 24 ? 55 57 41 54 41 56 41 57 48 8D AC 24 80 D0");
        Scr_GscObjLink = hook::library::QueryScanContainerSinglePtr<decltype(Scr_GscObjLink)>(
            "Scr_GscObjLink", "48 89 5C 24 ? 55 56 57 41 54 41 55 41 56 41 57 48 83 EC ? 48 8B DA 4C");
#endif
