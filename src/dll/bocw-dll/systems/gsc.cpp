...
namespace {
    ...
    byte* FindExport(scriptinstance::ScriptInstance inst, uint64_t target_script, uint32_t name_space, uint32_t name) {
        ...
        auto ite = std::find_if(
            reinterpret_cast<cw::T8GSCExport*>(obj->magic + obj->exports_tables),
            exportEnd,
    ...
    void VM_OP_LazyLink_Handler(
        scriptinstance::ScriptInstance inst, void* varInfo, cw::FunctionStack* fs_0, void* ctx, bool* terminate
    ) {
        struct LazyLinkData {
            uint32_t nameSpace;
            uint32_t name;
